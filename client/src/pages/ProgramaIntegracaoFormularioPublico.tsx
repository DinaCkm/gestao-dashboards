...
export default function ProgramaIntegracaoFormularioPublico() {
  ...
  const renderQuestion = (q: PublicQuestion) => {
    ...
    if (q.code === 'controle_gestor' || q.code === 'bem_anjo') {
      const lista = q.code === 'controle_gestor' ? opcoesAtivas.gestores : opcoesAtivas.anjos;
        ...
        <Card><CardContent className="space-y-6 pt-6">
          {pagina === 0 ? <>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="space-y-1 sm:col-span-2"><span className="text-sm font-medium">Nome completo do colaborador <span className="text-destructive">*</span></span><select value={draft.nomeColaborador} onChange={(e) => setDraft((d) => ({ ...d, nomeColaborador: e.target.value }))} className="w-full rounded-md border border-input bg-background px-3 py-2"><option value="">Selecione…</option>{opcoesAtivas.colaboradores.map((nome) => <option key={nome} value={nome}>{nome}</option>)}</select><span className="block text-xs text-muted-foreground">A lista mostra somente colaboradores ativos no Onboarding.</span></label>
