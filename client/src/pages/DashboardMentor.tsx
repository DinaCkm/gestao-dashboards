...
import { Users, Calendar, Building2, ClipboardList, TrendingUp, UserCheck, Eye, Info, AlertTriangle, Clock, Video, Settings, ArrowRight, ChevronDown, BookOpen } from "lucide-react";
...
function DashboardMentorContent() {
  ...
  // programId da empresa que o mentor atende (via mentor.getById)
  const { data: mentorDetalhes } = trpc.mentor.getById.useQuery(
    { id: selectedMentorId! },
    { enabled: !!selectedMentorId }
  );
  const mentorProgramId = (mentorDetalhes as any)?.programId as number | undefined;

  // Competências dos PDIs ativos da empresa do mentor
  const { data: competenciasPorEmpresa = [], isLoading: loadingCompetencias } =
    trpc.competencias.porEmpresaMacrociclo.useQuery(
      { programId: mentorProgramId ?? 0 },
      { enabled: !!mentorProgramId }
    );
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Competências em Desenvolvimento da Empresa (macrociclo ativo) */}
      {!!selectedMentorId && (
              ...
              <CardHeader className="cursor-pointer select-none hover:bg-muted/30 transition-colors rounded-t-xl">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <CardTitle className="text-base">Competências em Desenvolvimento da Empresa</CardTitle>
                  </div>
                  <ChevronDown className={`h-4 w-4 text-muted-foreground transition-transform duration-200 ${competenciasOpen ? 'rotate-180' : ''}`} />
                </div>
                <CardDescription>
                  Competências presentes nos PDIs ativos da empresa atendida — macrociclo atual
                </CardDescription>
              </CardHeader>
            </CollapsibleTrigger>
            <CollapsibleContent>
              <CardContent className="pt-0 pb-6 px-6">
                {!mentorProgramId && (
                  <p className="text-sm text-muted-foreground italic">Este mentor não está vinculado a uma empresa.</p>
                )}
                {mentorProgramId && loadingCompetencias && (
                  <p className="text-sm text-muted-foreground">Carregando...</p>
                )}
                {mentorProgramId && !loadingCompetencias && competenciasPorEmpresa.length === 0 && (
