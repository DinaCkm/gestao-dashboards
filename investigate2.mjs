...
async function main() {
  ...
  comps.forEach(c => {
    ...
    if (ciclos.has(key)) {
      ...
      const entry = { inicio: c.microInicio, fim: c.microTermino, obrig: [], opcional: [] };
      if (c.peso === 'opcional') entry.opcional.push(c.competenciaId);
      else entry.obrig.push(c.competenciaId);
  ...
  console.log('=== SESSOES DA JOSEANE ===');
  sessions.forEach(s => {
  ...
  ciclos.forEach((ciclo, key) => {
    ...
    console.log(`\nCiclo ${cicloNum} | ${ciclo.inicio} a ${ciclo.fim} | ${status} | obrig: ${temObrig} (${ciclo.obrig.length})`);
