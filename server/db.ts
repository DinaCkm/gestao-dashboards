...
export async function ensureDemoIntegrationProcessFixture() {
  if (!process.env.DATABASE_URL) return { ok: false, reason: "db_unavailable" as const };

  const demoLegacyId = "demo-integracao-ugp-20260925";
  const demoName = "[TESTE] Marina Demo Integração";
  const demoEmail = "marina.demo.integracao@example.com";
  const demoCpf = "88888888888";
  const demoGestor = "[TESTE] Rafael Gestor Demo";
  const demoGestorEmail = "rafael.gestor.demo@example.com";
  const demoAnjo = "[TESTE] Camila Anjo Demo";
  const demoAnjoEmail = "camila.anjo.demo@example.com";
  ...
  if (!program) {
    console.log("[DemoIntegracao] PROCESS_SKIP programa SEBRAE TO não localizado");
    return { ok: true, skipped: true as const };
  }
  ...
  try {
    ...
    const [existingRows]: any = await raw.execute(
    ...
    inicio.setDate(inicio.getDate() - 149);
    ...
    const d15 = addDays(inicio, 14);
    const d45 = addDays(inicio, 44);
    const d75 = addDays(inicio, 74);
    const d150 = addDays(inicio, 149);

    const feito: Record<string, any> = {};
    const itensSolicitacao = [
      "pos1-04","pos1-05","pos1-07",
      "pos2-04","pos2-05","pos2-07",
      "pos3-02","pos3-03","pos3-05",
      "pos4-03","pos4-04","pos4-06",
    ];
    for (const id of itensSolicitacao) {
      const data = id.startsWith("pos1") ? d15 : id.startsWith("pos2") ? d45 : id.startsWith("pos3") ? d75 : d150;
      feito[id] = { s: "ok", d: iso(data) };
    }

    const estado = {
      feito,
      alin: {
        "1": { realizado: true, dataReal: iso(d15), data: iso(d15), hora: "09:00" },
        "2": { realizado: true, dataReal: iso(d45), data: iso(d45), hora: "09:00" },
        "3": { realizado: true, dataReal: iso(d75), data: iso(d75), hora: "09:00" },
        "4": { realizado: true, dataReal: iso(d150), data: iso(d150), hora: "09:00" },
      },
      teste: {
        demoTag: "ugp_demo_20260925",
        empresaProgramId: Number(program.id),
      },
    };

    let processoId = Number(existingRows?.[0]?.id || 0);
    if (!processoId) {
      const [insertProcess]: any = await raw.execute(
        \`INSERT INTO programa_integracao_processos
         (legacyId,ordem,nome,cpf,email,emailCorporativo,tel,cargo,unidade,tipo,inicio,participacao,situacao,
          gestor,gestorEmail,gestorTel,anjo,anjoEmail,consultora,ugp,horarios,statusPdi,pendencias,statusCursos,
          consideracoes,notas,cor,estado,createdAt,updatedAt)
        ...
        [
          ...
          "Unidade de Desenvolvimento [TESTE]",
          iso(inicio),
          demoGestor,
          demoGestorEmail,
          "(63) 90000-0001",
          ...
          demoAnjoEmail,
          "[TESTE] Mentora Demo",
          "[TESTE] UGP Integração SEBRAE TO",
          "09:00\\n14:00\\n16:00",
          "Aguardando vínculo com PDI fictício",
          "",
          "Aguardando vínculo com Compliance fictício",
          "Processo fictício criado exclusivamente para demonstração da Visão UGP/RH.",
          "Não representa colaborador real.",
          "#6D4BA3",
          JSON.stringify(estado),
        ],
      );
      processoId = Number(insertProcess.insertId);
      if (!processoId) throw new Error("[DemoIntegracao] PROCESS_CREATE falhou ao obter ID");
      console.log("[DemoIntegracao] PROCESS_CREATED", JSON.stringify({ processoId, programId: Number(program.id) }));
    } else {
      await raw.execute(
        \`UPDATE programa_integracao_processos
         SET nome=?,cpf=?,email=?,emailCorporativo=?,cargo=?,unidade=?,tipo='Onboarding',inicio=?,
             situacao='ativo',gestor=?,gestorEmail=?,anjo=?,anjoEmail=?,estado=?,updatedAt=NOW()
         WHERE id=? AND legacyId=?\`,
        [
          demoName, demoCpf, demoEmail, demoEmail, "Analista de Projetos", "Unidade de Desenvolvimento [TESTE]",
          iso(inicio), demoGestor, demoGestorEmail, demoAnjo, demoAnjoEmail, JSON.stringify(estado),
          processoId, demoLegacyId,
        ],
      );
    }
    ...
    const pesquisaNotas: Record<number, number[]> = {
      1: [3,3,3,4,3, 3,3,3,3,3, 3,3,3, 3,3,3,3,3,3,3],
      2: [4,4,4,4,4, 4,4,4,4,4, 4,4,4, 4,3,4,4,4,4,4],
      3: [5,4,5,5,4, 5,4,5,4,4, 5,4,5, 5,2,5,4,5,5,5],
      4: [3,3,4,3,4, 4,3,4,3,3, 3,3,4, 4,4,4,3,4,4,4],
    };
    ...
    const cycleDates: Record<number, Date> = { 1: d15, 2: d45, 3: d75, 4: d150 };
    const pesquisaItems: Record<number, string> = { 1:"pos1-08", 2:"pos2-08", 3:"pos3-09", 4:"pos4-07" };
    const gestorItems: Record<number, string> = { 1:"pos1-09", 2:"pos2-09", 3:"pos3-10", 4:"pos4-08" };
    const anjoItems: Record<number, string> = { 1:"pos1-10", 2:"pos2-10", 3:"pos3-11", 4:"pos4-09" };

    const insertResponseIfMissing = async (args: {
      dedupeKey: string;
      formKey: string;
      ciclo: number;
      papel: string;
      ...
      const [exists]: any = await raw.execute(
        "SELECT id FROM programa_integracao_respostas WHERE dedupeKey=? LIMIT 1",
        [args.dedupeKey],
      );
      if (exists?.[0]?.id) return Number(exists[0].id);

      const submittedAt = addDays(cycleDates[args.ciclo] || inicio, 1);
      const [result]: any = await raw.execute(
        \`INSERT INTO programa_integracao_respostas
         (processoId,legacyRid,protocolo,dedupeKey,formKey,ciclo,papel,itemId,formVersion,statusVinculo,statusResposta,
          nomeColaborador,unidade,dataInicio,emailColaborador,nomeOrig,avaliador,respondentName,respondentEmail,source,
          media,alertas,answers,quandoOriginal,emOriginal,submittedAt,createdAt,updatedAt)
         VALUES (?,?,?,?,?,?,?,?,1,'vinculada','valido',?,?,?,?,?,?,?,?, 'admin',NULL,?,?,?, ?,?,?,NOW(),NOW())\`,
        [
          processoId,
          args.dedupeKey,
          \`DEMO-\${args.formKey.toUpperCase()}-\${args.papel || "COL"}-\${args.ciclo}\`,
          args.dedupeKey,
          args.formKey,
          args.ciclo,
          args.papel,
    ...
    for (const ciclo of [1,2,3,4]) {
      const respostasPesquisa = Object.fromEntries(
        pesquisaKeys.map((key, index) => [key, String(pesquisaNotas[ciclo][index])]),
      );
      await insertResponseIfMissing({
        dedupeKey: \`demo_ugp_20260925:pesquisa:\${ciclo}\`,
        formKey: "pesquisa",
        ciclo,
        papel: "Colaborador",
        itemId: pesquisaItems[ciclo],
        respondentName: demoName,
        respondentEmail: demoEmail,
        answers: respostasPesquisa,
      });

      for (const papel of ["Gestor","Anjo"] as const) {
        const isGestor = papel === "Gestor";
        const base = isGestor
          ? (ciclo === 1 ? 3 : ciclo === 2 ? 4 : ciclo === 3 ? 4 : 5)
          : (ciclo === 1 ? 3 : ciclo === 2 ? 3 : ciclo === 3 ? 4 : 3);
        const answers: Record<string, any> = {};
        avalKeys.forEach((key, index) => {
          const bonus = (index + ciclo) % 5 === 0 ? 1 : 0;
          answers[key] = String(Math.max(1, Math.min(5, base + bonus)));
        });
        answers.aval_desenvolvimento_conceito = ciclo === 1 ? "50%" : ciclo === 2 ? "75%" : ciclo === 3 ? "75%" : (isGestor ? "100%" : "75%");
        answers.aval_produtividade_conceito = answers.aval_desenvolvimento_conceito;
        answers.aval_conceito_geral = answers.aval_desenvolvimento_conceito;
        answers.aval_potencialidades = isGestor
          ? "Organização, proatividade, capacidade analítica e boa interação com a equipe."
          : "Cooperação, abertura para orientações e boa integração com os colegas.";
        answers.aval_menos_favoraveis = isGestor
          ? "Ainda pode ampliar segurança em decisões de maior complexidade."
          : "Em alguns momentos ainda busca validação antes de avançar.";
        answers.aval_orientacoes_desenvolvimento = "Aprofundar priorização, comunicação de avanços e tomada de decisão.";

        await insertResponseIfMissing({
          dedupeKey: \`demo_ugp_20260925:aval:\${papel.toLowerCase()}:\${ciclo}\`,
          formKey: "aval",
          ciclo,
          papel,
          itemId: isGestor ? gestorItems[ciclo] : anjoItems[ciclo],
          respondentName: isGestor ? demoGestor : demoAnjo,
          respondentEmail: isGestor ? demoGestorEmail : demoAnjoEmail,
          answers,
        });
      }
    }

    const [countRows]: any = await raw.execute(
      \`SELECT COUNT(*) AS total
       FROM programa_integracao_respostas
       WHERE processoId=? AND statusVinculo='vinculada' AND statusResposta<>'excluida'\`,
      [processoId],
    );
    const totalRespostas = Number(countRows?.[0]?.total || 0);
    if (totalRespostas < 12) {
      throw new Error(\`[DemoIntegracao] PROCESS_VERIFY esperadas ao menos 12 respostas; encontradas \${totalRespostas}\`);
    }

    await raw.commit();
    console.log("[DemoIntegracao] PROCESS_OK", JSON.stringify({
      processoId,
      legacyId: demoLegacyId,
      programId: Number(program.id),
    ...
    return { ok: true, processoId, respostas: totalRespostas };
  } catch (error) {
    try { await raw.rollback(); } catch {}
    throw error;
  } finally {
    try { await raw.end(); } catch {}
  }
}
