...
export async function ensureDemoIntegrationProcessFixture() {
  ...
  if (!processo) {
    await db.insert(programaIntegracaoProcessos).values({
    ...
    if (!processo) {
      throw new Error("[DemoIntegracao] MARINA_CREATE failed");
    }
  ...
  const ensureResponse = async (
    ...
    await db.insert(programaIntegracaoRespostas).values({
  ...
  const respostas = await db
    ...
    .where(
      and(
        eq(programaIntegracaoRespostas.processoId, processo.id),
