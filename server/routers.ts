...
import {
  calcularIndicadoresAlunoFiltrado,
  calcularPerformanceFiltrada,
  CompetenciaObrigatoria,
  CicloExecucaoData,
} from "./indicatorsCalculator";
import {
  calcularIndicadoresTodosAlunos,
  calcularIndicadoresAluno as calcularIndicadoresAlunoV2,
  agregarIndicadores,
  gerarDashboardGeral,
...
import {
  getRelatorioFinanceiroV2,
  getSessionTypePricingRules,
  createSessionTypePricingRule,
  updateSessionTypePricingRule,
  deleteSessionTypePricingRule,
  type TipoSessao,
} from "./financialCalculatorV2";
...
function calcularTempoMinimoExigidoSegundos(
  atividade: AtividadeTempoConfig
): number {
  if (
    atividade.tempoMinimoObrigatorioSegundos &&
    atividade.tempoMinimoObrigatorioSegundos > 0
  ) {
    return atividade.tempoMinimoObrigatorioSegundos;
  }

  const tempoEstimadoMinutos = Number(atividade.tempoEstimadoMinutos ?? 0);
  const percentualMinimoLiberacao = Number(
    atividade.percentualMinimoLiberacao ?? 60
  );

  if (tempoEstimadoMinutos <= 0) return 0;

  const calculado = Math.round(
    (tempoEstimadoMinutos * 60 * percentualMinimoLiberacao) / 100
  );
  return Math.max(0, calculado);
}

function normalizarSegundosHeartbeat(
...
function montarResumoTempo(
  ...
  const tempoAtivoAcumuladoSegundos = Number(
    progresso?.tempoAtivoAcumuladoSegundos ?? 0
  );
  const tempoRestanteSegundos = Math.max(
    0,
    tempoMinimoExigidoSegundos - tempoAtivoAcumuladoSegundos
  );

  const tempoCumprido =
    tempoMinimoExigidoSegundos <= 0 ||
    tempoAtivoAcumuladoSegundos >= tempoMinimoExigidoSegundos;
  ...
  const percentualTempoCumprido =
    tempoMinimoExigidoSegundos <= 0
      ...
      : Math.min(
          100,
          Math.round(
            (tempoAtivoAcumuladoSegundos / tempoMinimoExigidoSegundos) * 100
          )
        );
...
export const appRouter = router({
  ...
  auth: router({
    ...
    emailCpfLogin: publicProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        const result = await db.authenticateByEmailCpf(
          input.email,
          input.credential
        );
  ...
  uploads: router({
    ...
    uploadFile: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // Upload to S3
        const { fileKey, fileUrl } = await uploadExcelToStorage(
          buffer,
          input.fileName,
          ctx.user.id
        );
        ...
        if (input.fileType === "performance") {
          try {
            ...
            if (data.length >= 2) {
              const headers = (data[0] as unknown[]).map((h: unknown) =>
                String(h || "").trim()
              );
              ...
              for (const a of alunosList) {
                ...
                if (a.email)
                  alunoByEmail.set(a.email.toLowerCase().trim(), a.id);
                if (a.externalId)
                  alunoByExternalId.set(a.externalId.trim(), a.id);
              }
              ...
              const parseDecimalSafe = (
                val: string | undefined
              ): string | null => {
                if (!val || val === "-" || val.includes("Sem avalia"))
                  return null;
              ...
              for (let i = 1; i < data.length; i++) {
                ...
                alunoId =
                  alunoByExternalId.get(String(externalUserId).trim()) || null;
                if (!alunoId && userEmail) {
                  alunoId =
                    alunoByEmail.get(userEmail.toLowerCase().trim()) || null;
                }
                if (!alunoId && userName) {
                  alunoId =
                    alunoByName.get(userName.toLowerCase().trim()) || null;
                }
                ...
                if (turmaName) {
                  turmaId =
                    turmaByName.get(turmaName.toLowerCase().trim()) || null;
                ...
                if (compName) {
                  competenciaId =
                    compByName.get(compName.toLowerCase().trim()) || null;
                  if (!competenciaId) {
                    const baseName = compName
                      .replace(
                        /\s*-\s*(Master|Essential|Essencial|Basic|B.sica|Vis.o de Futuro|Jornada.*)$/i,
                        ""
                      )
                      .trim();
                    competenciaId =
                      compByName.get(baseName.toLowerCase()) || null;
                  }
                }
                ...
                records.push({
                  ...
                  turmaId,
                  externalTurmaId:
                    getXlsxVal(row, "Id Turma (agrupador 1)") || null,
                  turmaName: turmaName || null,
                  competenciaId,
                  externalCompetenciaId:
                    getXlsxVal(row, "Id Competência (agrupador 2)") || null,
                  ...
                  aulasDisponiveis: parseIntSafe(
                    getXlsxVal(row, "Aulas disponíveis")
                  ),
                  aulasConcluidas: parseIntSafe(
                    getXlsxVal(row, "Aulas concluídas")
                  ),
                  aulasEmAndamento: parseIntSafe(
                    getXlsxVal(row, "Aulas em andamento")
                  ),
                  aulasNaoIniciadas: parseIntSafe(
                    getXlsxVal(row, "Aulas não iniciadas")
                  ),
                  aulasAgendadas: parseIntSafe(
                    getXlsxVal(row, "Aulas agendadas")
                  ),
                  progressoTotal: parseIntSafe(
                    getXlsxVal(row, "Progresso Total")
                  ),
                  cargaHorariaTotal:
                    getXlsxVal(row, "Carga horária total") || null,
                  cargaHorariaConcluida:
                    getXlsxVal(row, "Carga horária concluída") || null,
                  progressoAulasDisponiveis: parseIntSafe(
                    getXlsxVal(row, "Progresso em aulas disponíveis")
                  ),
                  avaliacoesDiagnostico: parseIntSafe(
                    getXlsxVal(row, "Avaliações de diagnóstico")
                  ),
                  mediaAvaliacoesDiagnostico: parseDecimalSafe(
                    getXlsxVal(row, "Média das avaliações de diagnóstico")
                  ),
                  avaliacoesFinais: parseIntSafe(
                    getXlsxVal(row, "Avaliações finais")
                  ),
                  mediaAvaliacoesFinais: parseDecimalSafe(
                    getXlsxVal(row, "Média das avaliações finais")
                  ),
                  avaliacoesDisponiveis: parseIntSafe(
                    getXlsxVal(row, "Avaliações disponíveis")
                  ),
                  avaliacoesRespondidas: parseIntSafe(
                    getXlsxVal(row, "Avaliações respondidas")
                  ),
                  avaliacoesPendentes: parseIntSafe(
                    getXlsxVal(row, "Avaliações pendentes")
                  ),
                  avaliacoesAgendadas: parseIntSafe(
                    getXlsxVal(row, "Avaliações agendadas")
                  ),
                  mediaAvaliacoesDisponiveis: parseDecimalSafe(
                    getXlsxVal(row, "Média em avaliações disponíveis")
                  ),
                  mediaAvaliacoesRespondidas: parseDecimalSafe(
                    getXlsxVal(row, "Média em avaliações respondidas")
                  ),
                  concluidoDentroPrazo:
                    getXlsxVal(row, "Concluído dentro do prazo (%)") || null,
                  concluidoEmAtraso:
                    getXlsxVal(row, "Concluído em atraso (%)") || null,
                  naoConcluidoDentroPrazo:
                    getXlsxVal(row, "Não Concluído e dentro do prazo (%)") ||
                    null,
                  naoConcluidoEmAtraso:
                    getXlsxVal(row, "Não Concluído e em atraso (%)") || null,
              ...
              // Insert all records
              performanceInserted =
                await db.insertStudentPerformanceBatch(records);
            ...
            if (performanceInserted > 0) {
              ...
              const newTotal =
                (currentBatch?.totalRecords || 0) + performanceInserted;
...
Total de registros: ${files.reduce((sum, f) => sum + (f.rowCount || 0), 0)}`,
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        try {
          ...
          for (let i = 1; i < lines.length; i++) {
            ...
            const compName = getVal(
              values,
              colMap,
              "Compet\u00eancia (agrupador 2)"
            );
            ...
            records.push({
              ...
              turmaId,
              externalTurmaId:
                getVal(values, colMap, "Id Turma (agrupador 1)") || null,
              turmaName: turmaName || null,
              competenciaId,
              externalCompetenciaId:
                getVal(values, colMap, "Id Compet\u00eancia (agrupador 2)") ||
                null,
              competenciaName: compName || null,
              dataInicio: getVal(values, colMap, "Data de in\u00edcio") || null,
              dataConclusao:
                getVal(values, colMap, "Data de conclus\u00e3o") || null,
              totalAulas: parseIntSafe(
                getVal(values, colMap, "Total de aulas")
              ),
              aulasDisponiveis: parseIntSafe(
                getVal(values, colMap, "Aulas dispon\u00edveis")
              ),
              aulasConcluidas: parseIntSafe(
                getVal(values, colMap, "Aulas conclu\u00eddas")
              ),
              aulasEmAndamento: parseIntSafe(
                getVal(values, colMap, "Aulas em andamento")
              ),
              aulasNaoIniciadas: parseIntSafe(
                getVal(values, colMap, "Aulas n\u00e3o iniciadas")
              ),
              aulasAgendadas: parseIntSafe(
                getVal(values, colMap, "Aulas agendadas")
              ),
              progressoTotal: parseIntSafe(
                getVal(values, colMap, "Progresso Total")
              ),
              cargaHorariaTotal:
                getVal(values, colMap, "Carga hor\u00e1ria total") || null,
              cargaHorariaConcluida:
                getVal(values, colMap, "Carga hor\u00e1ria conclu\u00edda") ||
                null,
              progressoAulasDisponiveis: parseIntSafe(
                getVal(values, colMap, "Progresso em aulas dispon\u00edveis")
              ),
              avaliacoesDiagnostico: parseIntSafe(
                getVal(
                  values,
              ...
              mediaAvaliacoesDiagnostico: parseDecimalSafe(
                getVal(
                  values,
                  colMap,
                  "M\u00e9dia das avalia\u00e7\u00f5es de diagn\u00f3stico"
                )
              ),
              avaliacoesFinais: parseIntSafe(
                getVal(values, colMap, "Avalia\u00e7\u00f5es finais")
              ),
              mediaAvaliacoesFinais: parseDecimalSafe(
                getVal(
                  values,
              ...
              avaliacoesDisponiveis: parseIntSafe(
                getVal(values, colMap, "Avalia\u00e7\u00f5es dispon\u00edveis")
              ),
              avaliacoesRespondidas: parseIntSafe(
                getVal(values, colMap, "Avalia\u00e7\u00f5es respondidas")
              ),
              avaliacoesPendentes: parseIntSafe(
                getVal(values, colMap, "Avalia\u00e7\u00f5es pendentes")
              ),
              avaliacoesAgendadas: parseIntSafe(
                getVal(values, colMap, "Avalia\u00e7\u00f5es agendadas")
              ),
              mediaAvaliacoesDisponiveis: parseDecimalSafe(
                getVal(
                  values,
                  colMap,
                  "M\u00e9dia em avalia\u00e7\u00f5es dispon\u00edveis"
                )
              ),
              mediaAvaliacoesRespondidas: parseDecimalSafe(
                getVal(
                  values,
                  colMap,
                  "M\u00e9dia em avalia\u00e7\u00f5es respondidas"
                )
              ),
              concluidoDentroPrazo:
                getVal(values, colMap, "Conclu\u00eddo dentro do prazo (%)") ||
                null,
              concluidoEmAtraso:
                getVal(values, colMap, "Conclu\u00eddo em atraso (%)") || null,
              naoConcluidoDentroPrazo:
                getVal(
                  values,
                  colMap,
                  "N\u00e3o Conclu\u00eddo e dentro do prazo (%)"
                ) || null,
              naoConcluidoEmAtraso:
                getVal(
                  values,
                  colMap,
                  "N\u00e3o Conclu\u00eddo e em atraso (%)"
                ) || null,
              uploadId,
            });
          }
  ...
  dashboard: router({
    ...
    history: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        const scopeId =
          input.scope === "individual" ? ctx.user.id : input.scopeId;
  ...
  reports: router({
    generate: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (
          (input.type === "financeiro_mentora" ||
            input.type === "financeiro_empresa") &&
          ctx.user.role !== "admin"
        ) {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "Relatórios financeiros são restritos ao administrador",
          });
        }
        ...
        try {
          ...
          if (input.type === "individual" && input.scopeId) {
            ...
            const program = aluno.programId
              ? programMap.get(aluno.programId)
              : null;
            const turma = aluno.turmaId ? turmaMap.get(aluno.turmaId) : null;
            const consultor = aluno.consultorId
              ? consultorMap.get(aluno.consultorId)
              : null;
            const planoItems = allPlanoItems.filter(
            ...
            const dadosAluno = [
              {
                ...
                "Data da Última Mentoria": ultimaMentoria?.sessionDate
                  ? new Date(ultimaMentoria.sessionDate).toLocaleDateString(
                      "pt-BR"
                    )
                  : "Sem sessões",
            ...
            if (input.type === "manager") {
              ...
              if (userConsultorId) {
                reportAlunos = alunosList.filter(
                  a => a.consultorId === userConsultorId
                );
              } else if (ctx.user.programId) {
                reportAlunos = alunosList.filter(
                  a => a.programId === ctx.user.programId
                );
              } else {
            ...
            for (const session of mentoringSessions) {
              ...
              const prog = sessionAluno.programId
                ? programMap.get(sessionAluno.programId)
                : null;
              const turma = sessionAluno.turmaId
                ? turmaMap.get(sessionAluno.turmaId)
                : null;
              mentoriasV2.push({
                ...
                dataSessao: session.sessionDate
                  ? new Date(session.sessionDate)
                  : undefined,
                presenca: session.presence as "presente" | "ausente",
                atividadeEntregue: session.isAssessment
                  ? "sem_tarefa"
                  : ((session.taskStatus || "sem_tarefa") as
                      | "entregue"
                      | "nao_entregue"
                      | "sem_tarefa"),
            ...
            for (const ep of eventParticipations) {
              ...
              if (!epAluno) continue;
              const prog = epAluno.programId
                ? programMap.get(epAluno.programId)
                : null;
              eventosV2.push({
            ...
            // === UNIFICAÇÃO: Adicionar eventos ausentes (filtrado por macroInicio) ===
            {
              ...
              for (const _ep3 of eventParticipations) {
                if (!_epEvtIds3.has(_ep3.alunoId))
                  _epEvtIds3.set(_ep3.alunoId, new Set());
                _epEvtIds3.get(_ep3.alunoId)!.add(_ep3.eventId);
              }
              const _evtsByProg3 = new Map<
                number,
                Awaited<ReturnType<typeof db.getEventsByProgram>>
              >();
              for (const _prog3 of programsList) {
                _evtsByProg3.set(
                  _prog3.id,
                  await db.getEventsByProgramOrGlobal(_prog3.id)
                );
              }
                ...
                for (const _evt3 of _progEvts3) {
                  if (!_participated3.has(_evt3.id)) {
                    ...
                    eventosV2.push({
                      ...
                      dataEvento: _evt3.eventDate
                        ? new Date(_evt3.eventDate)
                        : undefined,
            ...
            for (const spRec of studentPerfRecs) {
              performanceV2.push(spRec);
            }
            ...
            for (const [, cases] of Array.from(casesMapReport.entries())) {
              casesDataReport.push(...cases);
            }
            ...
            const todosIndicadores = calcularIndicadoresTodosAlunos(
              ...
              eventosV2,
              performanceV2,
              ciclosPorAlunoReport,
              compIdToCodigoMapReport,
              casesDataReport,
              undefined,
              macrocicloPorAlunoReport
            );
            const indicadoresMap = new Map(
              todosIndicadores.map(i => [i.idUsuario, i])
            );
            ...
            if (input.type === "manager" && userConsultorIdForSheet) {
              ...
              const mentorSessions = mentoringSessions.filter(s =>
                reportAlunos.some(a => a.id === s.alunoId)
              );
              const totalPresente = mentorSessions.filter(
                s => s.presence === "presente"
              ).length;
              const totalAusente = mentorSessions.filter(
                s => s.presence === "ausente"
              ).length;
              const taxaPresenca =
                mentorSessions.length > 0
                  ? ((totalPresente / mentorSessions.length) * 100).toFixed(1)
                  : "0.0";
            ...
            const trilhaNameMap = new Map(
              allTrilhasForReport.map((t: any) => [t.id, t.name])
            );
            // Agrupar PDIs por aluno
            const pdisByAluno = new Map<number, typeof allAssessmentPdis>();
            for (const pdi of allAssessmentPdis) {
              if (!pdisByAluno.has(pdi.alunoId))
                pdisByAluno.set(pdi.alunoId, []);
            ...
            let allAssessmentComps: {
              id: number;
              assessmentPdiId: number;
              competenciaId: number;
              microInicio: Date | string | null;
              microTermino: Date | string | null;
            }[] = [];
            if (allPdiIds.length > 0) {
              allAssessmentComps =
                await db.getAllAssessmentCompetenciasForReport();
            }
            // Buscar nomes das competências
            const allCompetenciasForReport = await db.getAllCompetencias();
            const compNameMap = new Map(
              allCompetenciasForReport.map((c: any) => [c.id, c.nome])
            );
            ...
            for (const comp of allAssessmentComps) {
              if (!compsByPdiId.has(comp.assessmentPdiId))
                compsByPdiId.set(comp.assessmentPdiId, []);
              compsByPdiId.get(comp.assessmentPdiId)!.push(comp);
            }

            // Sheet 1: Alunos com Indicadores V2
            const sheetName1 =
              input.type === "manager" ? "Equipe" : "Todos os Alunos";
            const alunosComIndicadores = reportAlunos.map(a => {
              ...
              const mentor = a.consultorId
                ? consultorMap.get(a.consultorId)
                : null;
              ...
              const alunoSessoes = mentoringSessions
                ...
                .sort((sa, sb) => {
                  const da = sa.sessionDate
                    ? new Date(sa.sessionDate).getTime()
                    : 0;
                  const db2 = sb.sessionDate
                    ? new Date(sb.sessionDate).getTime()
                    : 0;
                  return db2 - da;
                });
              const ultimaMentoria = alunoSessoes[0];
              ...
              for (const pdi of alunoPdis) {
                ...
                const compsComDatas = comps.filter(
                  c => c.microInicio || c.microTermino
                );
                for (const comp of compsComDatas) {
                  const compNome =
                    compNameMap.get(comp.competenciaId) ||
                    `Comp ${comp.competenciaId}`;
                  microciclosArr.push(
                    `${compNome} (${fmtDate(comp.microInicio)} - ${fmtDate(comp.microTermino)})`
                  );
                }
              }
              const microciclosStr =
                microciclosArr.length > 0
                  ? microciclosArr.join(" | ")
                  : "Sem microciclos";
              ...
              return {
                ...
                "Última Mentoria": ultimaMentoria?.sessionDate
                  ? new Date(ultimaMentoria.sessionDate).toLocaleDateString(
                      "pt-BR"
                    )
                  : "Sem sessões",
                "Ind.1 Webinars (%)": ind
                  ? Math.round(ind.consolidado.ind1_webinars)
                  : 0,
                "Ind.2 Avaliações (%)": ind
                  ? Math.round(ind.consolidado.ind2_avaliacoes)
                  : 0,
                "Ind.3 Competências (%)": ind
                  ? Math.round(ind.consolidado.ind3_competencias)
                  : 0,
                "Ind.4 Tarefas (%)": ind
                  ? Math.round(ind.consolidado.ind4_tarefas)
                  : 0,
                "Ind.5 Engajamento (%)": ind
                  ? Math.round(ind.consolidado.ind5_engajamento)
                  : 0,
                "Ind.6 Case (%)": ind
                  ? Math.round(ind.consolidado.ind6_aplicabilidade)
                  : 0,
                "Ind.7 Engajamento Final (%)": ind
                  ? Math.round(ind.consolidado.ind7_engajamentoFinal)
                  : 0,
            ...
            const reportMentorias = mentoringSessions
              ...
              .map(s => {
                ...
                const prog = al?.programId
                  ? programMap.get(al.programId)
                  : null;
                return {
                  Aluno: al?.name || "",
                  ...(input.type === "admin"
            ...
            for (const a of reportAlunos) {
              ...
              if (!ind) continue;
              const allCiclos = [
                ...ind.ciclosFinalizados,
                ...ind.ciclosEmAndamento,
              ];
            ...
            for (const s of mentoringSessions) {
              ...
              const program = aluno?.programId
                ? programMap.get(aluno.programId)
                : null;
              const valorPadrao = consultor.valorSessao
                ? Number(consultor.valorSessao)
                : 0;
              ...
              const matchingRule = rules.find(
                r => sessionNum >= r.sessionFrom && sessionNum <= r.sessionTo
              );
              const valorSessao = matchingRule
                ? Number(matchingRule.valor)
                : valorPadrao;
            ...
            const resumoMentoras = Object.values(mentoraSummary)
              .map(m => ({
                Mentora: m.nome,
                "Total de Sessões": m.sessoes.length,
                "Valor Total (R$)": m.sessoes
                  .reduce((sum, s) => sum + s.valor, 0)
                  .toFixed(2),
                "Alunos Atendidos": Array.from(
                  new Set(m.sessoes.map(s => s.alunoNome))
                ).length,
                Empresas: Array.from(
                  new Set(m.sessoes.map(s => s.empresaNome))
                ).join(", "),
              }))
              .sort(
                (a, b) =>
                  parseFloat(b["Valor Total (R$)"]) -
                  parseFloat(a["Valor Total (R$)"])
              );

            const totalGeralMentoras = resumoMentoras.reduce(
              (sum, m) => sum + parseFloat(m["Valor Total (R$)"]),
              0
            );
            resumoMentoras.push({
              Mentora: "TOTAL GERAL",
              "Total de Sessões": resumoMentoras.reduce(
                (sum, m) => sum + m["Total de Sessões"],
                0
              ),
            ...
            detalheMentora.sort((a, b) =>
              a["Mentora"].localeCompare(b["Mentora"])
            );
            ...
            for (const s of mentoringSessions) {
              ...
              if (!program) continue;
              const consultor = s.consultorId
                ? consultorMap.get(s.consultorId)
                : null;
              const valorPadrao = consultor?.valorSessao
                ? Number(consultor.valorSessao)
                : 0;
              ...
              // Calcular valor usando precificação flexível
              const rules = s.consultorId
                ? pricingMap.get(s.consultorId) || []
                : [];
              const sessionNum = s.sessionNumber || 0;
              const matchingRule = rules.find(
                r => sessionNum >= r.sessionFrom && sessionNum <= r.sessionTo
            ...
            const resumoEmpresas = Object.values(empresaSummary)
              .map(e => ({
                Empresa: e.nome,
                "Total de Sessões": e.sessoes.length,
                "Valor Total (R$)": e.sessoes
                  .reduce((sum, s) => sum + s.valor, 0)
                  .toFixed(2),
                "Alunos Atendidos": Array.from(
                  new Set(e.sessoes.map(s => s.alunoNome))
                ).length,
                Mentoras: Array.from(
                  new Set(e.sessoes.map(s => s.mentoraNome))
                ).join(", "),
              }))
              .sort(
                (a, b) =>
            ...
            const totalGeralEmpresas = resumoEmpresas.reduce(
              (sum, e) => sum + parseFloat(e["Valor Total (R$)"]),
              0
            );
            resumoEmpresas.push({
              Empresa: "TOTAL GERAL",
              "Total de Sessões": resumoEmpresas.reduce(
                (sum, e) => sum + e["Total de Sessões"],
                0
              ),
      ...
      .mutation(async ({ input }) => {
        ...
        if (!success) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message:
              "Não é possível excluir trilha com competências vinculadas",
          });
        }
        return { success: true };
      }),
  ...
  planoIndividual: router({
    ...
    addMultiple: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        const success = await db.addCompetenciasToPlano(
          input.alunoId,
  ...
  alunos: router({
    ...
    byConsultor: protectedProcedure
      .input(
        z.object({ consultorId: z.number(), programId: z.number().optional() })
      )
      .query(async ({ input }) => {
        return await db.getAlunosByConsultor(
          input.consultorId,
          input.programId
        );
      }),
  ...
  indicadores: router({
    ...
    visaoGeral: protectedProcedure.query(async ({ ctx }) => {
      ...
      for (const participation of eventParticipations) {
        ...
        eventos.push({
          ...
          dataEvento: participation.eventDate
            ? new Date(participation.eventDate)
            : undefined,
      ...
      // === UNIFICAÇÃO: Adicionar eventos ausentes (filtrado por macroInicio) ===
      {
        ...
        for (const _ep of eventParticipations) {
          if (!_epEvtIds.has(_ep.alunoId))
            _epEvtIds.set(_ep.alunoId, new Set());
          _epEvtIds.get(_ep.alunoId)!.add(_ep.eventId);
        }
        const _evtsByProg = new Map<
          number,
          Awaited<ReturnType<typeof db.getEventsByProgram>>
        >();
        for (const _prog of programsList) {
          _evtsByProg.set(
            _prog.id,
            await db.getEventsByProgramOrGlobal(_prog.id)
          );
        }
          ...
          for (const _evt of _progEvts) {
            if (!_participated.has(_evt.id)) {
              ...
              eventos.push({
                ...
                dataEvento: _evt.eventDate
                  ? new Date(_evt.eventDate)
                  : undefined,
                presenca: "ausente" as const,
              });
            }
      ...
      const existingPerfKeys = new Set(
        performance.map(p => `${p.idUsuario}|${p.idCompetencia}`)
      );
      ...
      const isGerente =
        ...
        (ctx.user as any).consultorRole === "gerente";
      if (isGerente && ctx.user.programId) {
        ...
        if (userProgram) {
          ...
          const filteredMentorias = mentorias.filter(
            m => m.empresa === userProgram.name
          );
          const filteredEventos = eventos.filter(
            e => e.empresa === userProgram.name
          );
          const filteredPerformance = performance.filter(p => {
            const aluno = alunosList.find(
              a => a.externalId === p.idUsuario || String(a.id) === p.idUsuario
            );
    ...
    porEmpresa: managerProcedure
      ...
      .query(async ({ input }) => {
        ...
        for (const [, cases] of Array.from(casesMapEmp.entries())) {
          casesDataEmp.push(...cases);
        }
        ...
        const trilhaLookupMap = new Map(
          allTrilhasLookup.map(t => [t.id, t.name])
        );
        const pdisCongeladosPorAluno = new Map<
          number,
          { trilhaNome: string; motivoCongelamento: string | null }[]
        >();
        for (const pdi of allAssessmentPdis) {
          if (pdi.status === "congelado") {
            if (!pdisCongeladosPorAluno.has(pdi.alunoId))
              pdisCongeladosPorAluno.set(pdi.alunoId, []);
        ...
        const alunosEnriquecidos = dashboard.alunos.map(ind => {
          const alunoDb = alunosList.find(
            a => (a.externalId || String(a.id)) === ind.idUsuario
          );
          const turma = alunoDb?.turmaId ? turmaMap.get(alunoDb.turmaId) : null;
          const mentor = alunoDb?.consultorId
            ? consultorMap.get(alunoDb.consultorId)
            : null;
          ...
          const planoItems = allPlanoItems.filter(
            p => alunoDb && p.alunoId === alunoDb.id
          );
          ...
          // Ciclo atual
          const cicloAtual =
            ind.ciclosEmAndamento?.[0]?.nomeCiclo ||
            (ind.ciclosFinalizados?.length
              ? `${ind.ciclosFinalizados.length} ciclo(s) finalizado(s)`
              : "Nenhum ciclo");

          // Trilhas reais do aluno (via assessment_pdi)
          const trilhasReais = alunoDb
            ? trilhasReaisPorAluno.get(alunoDb.id) || [trilhaNome]
            : [trilhaNome];

          // PDIs congelados do aluno
          const pdisCongelados = alunoDb
            ? pdisCongeladosPorAluno.get(alunoDb.id) || []
            : [];
          ...
          return {
            ...
            competenciasComNota: competencias.filter(c => c.nota !== null)
              .length,
            pdisCongelados,
    ...
    porTurma: managerProcedure
      ...
      .query(async ({ input }) => {
        ...
        for (const [, cases] of Array.from(casesMapTurma.entries())) {
          casesDataTurma.push(...cases);
        }
        ...
        const indicadores = calcularIndicadoresTodosAlunos(
          ...
          casesDataTurma,
          undefined,
          macrocicloPorAlunoTurma
        );
        const agregado = agregarIndicadores(
          indicadores,
          "turma",
          String(input.turmaId)
        );
        const alunos = indicadores.filter(
          i => i.turma === String(input.turmaId)
        );
    ...
    porAluno: protectedProcedure
      ...
      .query(async ({ input }) => {
        ...
        for (const [, cases] of Array.from(casesMapInd.entries())) {
          casesDataInd.push(...cases);
        }
        ...
        const indicadores = calcularIndicadoresTodosAlunos(
          ...
          casesDataInd,
          undefined,
          macrocicloPorAlunoInd
        );
        const alunoIndicadores = indicadores.find(
          i => i.idUsuario === input.alunoId
        );
    ...
    enviarLembreteEngajamento: managerProcedure
      .input(
        z.object({
          ...
          engajamentoFinal: z.number().min(0).max(100),
        })
      )
      .mutation(async ({ ctx, input }) => {
        const isGestor =
        ...
        if (!isGestor || !ctx.user.programId) {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "Acesso restrito ao gestor da empresa.",
          });
        }

        const alunosEmpresa = await db.getAlunos(ctx.user.programId);
        const aluno = alunosEmpresa.find(
          a => (a.externalId || String(a.id)) === input.alunoIdUsuario
        );

        if (!aluno) {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "Aluno não pertence à empresa do gestor.",
          });
        }
        ...
        if (!aluno.email) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Aluno sem e-mail cadastrado.",
          });
        }

        const emailData = buildLembreteEngajamentoEmail({
          engajamentoFinal: input.engajamentoFinal,
        });
        ...
        if (dbConn) {
          await dbConn.insert(emailAlertasLog).values({
            alunoId: aluno.id,
            consultorId: aluno.consultorId || 0,
            tipoAlerta: "ranking_engajamento_lembrete",
            diasSemSessao: 0,
            emailEnviado: envio.success ? 1 : 0,
            erro: envio.success
              ? null
              : envio.error || "Falha no envio do lembrete de ranking",
          });
        }
        ...
        if (!envio.success) {
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message: envio.error || "Falha ao enviar e-mail.",
          });
        }
    ...
    exportarRankingEngajamentoExcel: managerProcedure
        ...
        z.object({
          ...
          rows: z.array(
            z.object({
              ...
              pessoa: z.string(),
              turma: z.string(),
              ind1Webinars: z.number(),
              ind2Avaliacoes: z.number(),
              ind3Competencias: z.number(),
              ind4Tarefas: z.number(),
              ind5Engajamento: z.number(),
              indMediaEngajamentoFinal: z.number(),
            })
          ),
        })
      )
      .mutation(async ({ ctx, input }) => {
        ...
        const idsEmpresa = new Set(
          alunosEmpresa.map(a => a.externalId || String(a.id))
        );

        for (const row of input.rows) {
          if (!idsEmpresa.has(row.alunoIdUsuario)) {
            throw new TRPCError({
              code: "FORBIDDEN",
              message: "Tentativa de exportar aluno fora do escopo da empresa.",
            });
          }
        }
        ...
        const exportRows = input.rows.map(row => ({
          ...
          Pessoa: row.pessoa,
          Turma: row.turma,
          "Ind. 1: Webinars": `${Math.round(row.ind1Webinars)}%`,
          "Ind. 2: Avaliações": `${Math.round(row.ind2Avaliacoes)}%`,
          "Ind. 3: Competências": `${Math.round(row.ind3Competencias)}%`,
          "Ind. 4: Tarefas": `${Math.round(row.ind4Tarefas)}%`,
          "Ind. 5: Engajamento": `${Math.round(row.ind5Engajamento)}%`,
          "Ind. Média: Engajamento Final": `${Math.round(row.indMediaEngajamentoFinal)}%`,
        }));
        ...
        const safeEmpresa = input.empresaNome
          .normalize("NFD")
          .replace(/[̀-ͯ]/g, "")
    ...
    performanceFiltrada: protectedProcedure
      ...
      .query(async ({ input }) => {
        ...
        const competenciasObrigatorias =
          await db.getCompetenciasObrigatoriasAluno(input.alunoId);
        ...
        for (const session of mentoringSessions) {
          const sessionAluno = alunoMap.get(session.alunoId);
          if (!sessionAluno) continue;
          const program = sessionAluno.programId
            ? programMap.get(sessionAluno.programId)
            : null;
        ...
        for (const participation of eventParticipations) {
          ...
          const program = partAluno.programId
            ? programMap.get(partAluno.programId)
            : null;
        ...
        const compObrigatorias: CompetenciaObrigatoria[] =
          competenciasObrigatorias.map(c => ({
        ...
        return {
          ...
          planoIndividual: {
            ...
            percentualAprovacao:
              indicadores.performanceFiltrada.percentualAprovacao,
    ...
    meuDashboard: protectedProcedure.query(async ({ ctx }) => {
      ...
      if (!aluno) {
        return {
          ...
          message: "Nenhum perfil de aluno vinculado a esta conta.",
        };
      }

      // Buscar competências obrigatórias do plano individual
      const competenciasObrigatorias =
        await db.getCompetenciasObrigatoriasAluno(aluno.id);

      // Buscar dados globais para cálculo de indicadores e ranking
      const allSessions = await db.getAllMentoringSessions();
      const allEventParticipations =
        await db.getAllEventParticipationWithDate();
      const alunosList = await db.getAlunos();
      const programsList = await db.getPrograms();
      const turmasList = await db.getTurmas();
      ...
      for (const ep of allEventParticipations) {
        const epAluno = alunoMap.get(ep.alunoId);
        if (!epAluno) continue;
        const program = epAluno.programId
          ? programMap.get(epAluno.programId)
          : null;
        eventos.push({
          idUsuario: epAluno.externalId || String(epAluno.id),
      ...
      for (const a of alunosList) {
        ...
        const alunoParticipatedEvents =
          eventParticipationEventIds.get(a.id) || new Set();
      ...
      const allEvents = aluno.programId
        ? await db.getEventsByProgramOrGlobal(aluno.programId)
        : [];
      ...
      for (const [, cases] of Array.from(casesMapAll.entries())) {
      ...
      if (aluno.programId) {
        ...
        const posicao =
          alunosEmpresaV2.findIndex(i => i.idUsuario === idUsuario) + 1;
      ...
      const alunoIndicadoresV2Global = todosIndicadoresV2.find(
        i => i.idUsuario === idUsuario
      );
      ...
      return {
        ...
        aluno: {
          ...
          cicloAtual:
            indicadores.ciclosEmAndamento?.[0]?.nomeCiclo ||
            (indicadores.ciclosFinalizados?.length
              ? `${indicadores.ciclosFinalizados.length + (indicadores.ciclosEmAndamento?.length || 0)} ciclo(s)`
              : "Nenhum ciclo"),
        ...
        indicadores: {
          ...
          participacaoMentorias:
            alunoIndicadoresV2Global?.participacaoMentorias ??
            indicadores.participacaoMentorias,
          atividadesPraticas:
            alunoIndicadoresV2Global?.atividadesPraticas ??
            indicadores.atividadesPraticas,
          engajamento:
            alunoIndicadoresV2Global?.engajamento ?? indicadores.engajamento,
          performanceCompetencias:
            alunoIndicadoresV2Global?.performanceCompetencias ??
            indicadores.performanceCompetencias,
          performanceAprendizado:
            alunoIndicadoresV2Global?.performanceAprendizado ??
            indicadores.performanceAprendizado,
          participacaoEventos:
            alunoIndicadoresV2Global?.participacaoEventos ??
            indicadores.participacaoEventos,
          performanceGeral:
            alunoIndicadoresV2Global?.performanceGeral ??
            indicadores.performanceGeral,
          notaFinal:
            alunoIndicadoresV2Global?.notaFinal ?? indicadores.notaFinal,
          classificacao:
            alunoIndicadoresV2Global?.classificacao ??
            indicadores.classificacao,
          totalMentorias:
            alunoIndicadoresV2Global?.totalMentorias ??
            indicadores.totalMentorias,
          mentoriasPresente:
            alunoIndicadoresV2Global?.mentoriasPresente ??
            indicadores.mentoriasPresente,
          totalAtividades:
            alunoIndicadoresV2Global?.totalAtividades ??
            indicadores.totalAtividades,
          atividadesEntregues:
            alunoIndicadoresV2Global?.atividadesEntregues ??
            indicadores.atividadesEntregues,
          totalEventos:
            alunoIndicadoresV2Global?.totalEventos ?? indicadores.totalEventos,
          eventosPresente:
            alunoIndicadoresV2Global?.eventosPresente ??
            indicadores.eventosPresente,
          totalCompetencias:
            alunoIndicadoresV2Global?.totalCompetencias ??
            indicadores.totalCompetencias,
          competenciasAprovadas:
            alunoIndicadoresV2Global?.competenciasAprovadas ??
            indicadores.competenciasAprovadas,
          mediaEngajamentoRaw:
            alunoIndicadoresV2Global?.mediaEngajamentoRaw ??
            indicadores.mediaEngajamentoRaw,
        ...
        pdisCongelados: (await db.getAssessmentsByAluno(aluno.id))
        ...
        ciclosDetalhados: await (async () => {
          ...
          const perfByCodigoMap = new Map<
            string,
            (typeof studentPerfRecords)[0]
          >();
          ...
          return ciclos.map(c => {
            ...
            return {
              ...
              dataInicio:
                typeof c.dataInicio === "string"
                  ? c.dataInicio
                  : new Date(c.dataInicio).toISOString().split("T")[0],
              dataFim:
                typeof c.dataFim === "string"
                  ? c.dataFim
                  : new Date(c.dataFim).toISOString().split("T")[0],
              status,
              competencias: c.competencias.map(comp => {
                const planoItem = planoItems.find(
                  p => p.competenciaId === comp.competenciaId
                );
                // Buscar nota do student_performance pelo codigoIntegracao
                let nota: number | null = planoItem?.notaAtual
                  ? parseFloat(planoItem.notaAtual)
                  : null;
                let progressoPlataforma: number | null = null;
                const codigoInt =
                  planoItem?.competenciaCodigo || comp.competenciaCodigo;
                if (codigoInt) {
                ...
                const meta = planoItem?.metaNota
                  ? parseFloat(planoItem.metaNota)
                  : 7;
                return {
        ...
        indicadoresV2: {
          ...
          alertaCasePendente: await (async () => {
            ...
            const trilhaNameToId = new Map(
              allTrilhasForAlert.map(t => [t.name.toLowerCase(), t.id])
            );
            return indicadoresV2.alertaCasePendente.map(a => ({
              ...a,
              trilhaId:
                trilhaNameToId.get(a.trilhaNome?.toLowerCase() || "") || null,
            }));
          })(),
        },
        ...
        aplicabilidadePratica: await (async () => {
          ...
          const sessoesComAplic = todasSessoes.filter(s => {
            ...
            return (
              dataSession &&
              dataSession >= CUTOFF_DATE &&
              (s.notaAlunoAplicabilidade !== null ||
                s.notaMentoraAplicabilidade !== null)
            );
          });
          ...
          const casesComAplic = casesAluno.filter(c => {
            ...
            return (
              dataCase &&
              dataCase >= CUTOFF_DATE &&
              c.entregue === 1 &&
              ((c as any).notaAlunoAplicabilidade !== null ||
                (c as any).notaMentoraAplicabilidade !== null)
            );
          });
          ...
          let somaNotaAluno = 0,
            countNotaAluno = 0;
          let somaNotaMentora = 0,
            countNotaMentora = 0;

          for (const s of sessoesComAplic) {
            if (
              s.notaAlunoAplicabilidade !== null &&
              s.notaAlunoAplicabilidade !== undefined
            ) {
            ...
            if (
              s.notaMentoraAplicabilidade !== null &&
              s.notaMentoraAplicabilidade !== undefined
            ) {
          ...
          const mediaAluno =
            countNotaAluno > 0 ? somaNotaAluno / countNotaAluno : null;
          const mediaMentora =
            countNotaMentora > 0 ? somaNotaMentora / countNotaMentora : null;
          ...
          const percentual =
            notaFinal !== null ? Math.round(notaFinal * 10) : 0;
          ...
          return {
            ...
            bonusEngajamento,
            mediaAluno:
              mediaAluno !== null ? Math.round(mediaAluno * 100) / 100 : null,
            mediaMentora:
              mediaMentora !== null
                ? Math.round(mediaMentora * 100) / 100
                : null,
  ...
  mentor: router({
    ...
    gestorTeamStats: managerProcedure
    ...
    notificarCicloQuaseFechando: managerProcedure.mutation(async () => {
      ...
      if (alunosFalta1.length === 0 && alunosCicloCompleto.length === 0) {
        return {
          sent: false,
          message:
            "Nenhum aluno a 1 sessão de fechar o ciclo ou com ciclo completo.",
        };
      }
      ...
      return {
        sent,
        alunosFalta1: alunosFalta1.length,
        alunosCicloCompleto: alunosCicloCompleto.length,
      };
    }),
    ...
    updateSession: protectedProcedure
      .input(
        z.object({
          ...
          taskStatus: z
            .enum(["entregue", "nao_entregue", "sem_tarefa"])
            .optional(),
          ...
          taskMode: z
            .enum(["biblioteca", "personalizada", "livre", "sem_tarefa"])
            .optional(),
          notaMentoraAplicabilidade: z
            .number()
            .min(0)
            .max(10)
            .nullable()
    ...
    createSession: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const consultor = consultors.find(
          c =>
            c.loginId === ctx.user.openId ||
            (ctx.user.consultorId && c.id === ctx.user.consultorId)
        );
        ...
        if (!consultorId) {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "Você não está vinculado como mentor",
          });
        }
        ...
        const sessionId = await db.createMentoringSession({
          ...
          aplicabilidadeAvaliadaEm:
            input.notaMentoraAplicabilidade != null ? new Date() : null,
        ...
        try {
          ...
          const alunoUser = allUsers.find(
            (u: any) => u.alunoId === input.alunoId
          );
          if (alunoUser) {
          ...
          /* notificação não deve bloquear registro */
        }
    ...
    validateTask: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!consultor && ctx.user.role !== "admin") {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "Apenas mentores podem validar atividades",
          });
        }
        ...
        if (session.taskStatus !== "entregue") {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Só é possível validar atividades com status ENTREGUE",
          });
        }
    ...
    getSubmissionDetail: protectedProcedure
      ...
      .query(async ({ input }) => {
        ...
        const task = session.taskId
          ? await db.getTaskLibraryById(session.taskId)
          : null;
        const comments = await db.getCommentsBySessionId(input.sessionId);
        const allAlunos = await db.getAlunos();
        const aluno = allAlunos.find(a => a.id === session.alunoId);
        const consultors = await db.getConsultors();
        const validador = session.validatedBy
          ? consultors.find(c => c.id === session.validatedBy)
          : null;
        return {
    ...
    taskSubmissions: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        const sessionsWithTask = sessions.filter(
          s => s.taskId !== null && s.taskId !== undefined
        );
    ...
    saveDateAvailability: managerProcedure
      ...
      .mutation(async ({ input }) => {
        return await db.saveMentorDateAvailability(
          input.consultorId,
          input.slots
        );
      }),
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (!matchingSlot) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message:
              "O mentor não tem disponibilidade neste dia/horário. Verifique a agenda.",
          });
        }

        // Verificar se o horário já não está ocupado
        const existing = await db.checkAppointmentConflict(
          input.consultorId,
          input.scheduledDate,
          input.startTime
        );
    ...
    respondToInvite: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        return await db.respondToAppointmentInvite(
          input.appointmentId,
          alunoId,
          input.response,
          input.notes || null
    ...
    updateAppointment: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (userRole !== "admin" && userRole !== "manager") {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "Sem permissão para reagendar",
          });
        }
        ...
        if (result.success && oldAppointment) {
          try {
            const participants = await db.getAppointmentParticipants(
              input.appointmentId
            );
            const mentor = oldAppointment.consultorId
              ? await db.getConsultorById(oldAppointment.consultorId)
              : null;
            const mentorName = mentor?.name || "Mentor(a)";
            const oldDateStr = new Date(
              oldAppointment.scheduledDate + "T12:00:00"
            ).toLocaleDateString("pt-BR");
            const newDateStr = new Date(
              input.scheduledDate + "T12:00:00"
            ).toLocaleDateString("pt-BR");
...
<body style="margin:0;padding:0;background-color:#f4f6f8;font-family:'Segoe UI',Tahoma,Geneva,Verdana,sans-serif;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#f4f6f8;padding:40px 20px;">
        ...
        <tr><td style="background-color:#f9fafb;padding:20px 40px;text-align:center;border-top:1px solid #e5e7eb;">
          <p style="color:#9ca3af;font-size:12px;margin:0;">ECOSSISTEMA DO BEM — Plataforma de Desenvolvimento e Mentoria</p>
              ...
              await sendEmail({
                ...
                subject,
                html,
                text: `Sessão reagendada: de ${oldDateStr} ${oldAppointment.startTime} para ${newDateStr} ${input.startTime}`,
              }).catch(() => {});
            }
          } catch (emailErr) {
    ...
    getAvailableSlots: protectedProcedure
      ...
      .query(async ({ input }) => {
        ...
        const daySlots = availability.filter(
          a => a.dayOfWeek === dayOfWeek && a.isActive === 1
        );
        ...
        const appointments = await db.getAppointmentsForDate(
          input.consultorId,
          input.date
        );
    ...
    dashboardGeral: managerProcedure.query(async () => {
      ...
      const mentoresAtivos = consultors.filter(
        c => c.role === "mentor" && c.isActive === 1
      );
      const allStats = [];
    ...
    relatorioFinanceiro: managerProcedure
      ...
      .query(async ({ input }) => {
        return await db.getRelatorioFinanceiroMentorias(
          input?.dateFrom,
          input?.dateTo
        );
      }),
    ...
    relatorioFinanceiroV2: managerProcedure
      ...
      .query(async ({ input }) => {
        ...
        return await getRelatorioFinanceiroV2(
          dbConn,
          input?.dateFrom,
          input?.dateTo
        );
    ...
    getPricingRulesV2: adminProcedure.query(async () => {
      const dbConn = await getDb();
      if (!dbConn)
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const conflito = existing.find(
          (r: any) =>
            ...
            (!r.validoAte ||
              !input.validoDesde ||
              String(r.validoAte) >= input.validoDesde) &&
            (!input.validoAte ||
              !r.validoDesde ||
              input.validoAte >= String(r.validoDesde))
        );
        if (conflito) {
          throw new TRPCError({
            code: "CONFLICT",
    ...
    updatePricingRuleV2: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (
          ...
          data.consultorId !== undefined ||
          data.tipoSessao !== undefined
        ) {
          const existing = await getSessionTypePricingRules(dbConn);
          ...
          const checkDesde =
            data.validoDesde ??
            (current?.validoDesde ? String(current.validoDesde) : null);
          const checkAte =
            data.validoAte !== undefined
              ? data.validoAte
              : current?.validoAte
                ? String(current.validoAte)
                : null;
          const conflito = existing.find(
            (r: any) =>
              r.id !== id &&
    ...
    relatorioDetalhadoMentor: managerProcedure
      ...
      .query(async ({ input }) => {
        ...
        const report = await getRelatorioFinanceiroV2(
          dbConn,
          input.dateFrom,
          input.dateTo
        );
        const mentorData = report.mentores.find(
          m => m.consultorId === input.consultorId
        );

        // Buscar agendamentos do mentor (já vem com participants enriquecidos)
        const allAppointments = await db.getMentorAppointments(
          input.consultorId
        );
        ...
        if (mentorData) {
          for (const s of mentorData.sessoes) {
            const appt = s.appointmentId
              ? allAppointments.find(a => a.id === s.appointmentId)
              : null;
            const participants = appt
              ? participantsByAppt.get(appt.id) || []
              : [];
        ...
        const gapsMentor = report.gapsAgendamento.filter(
          g => g.consultorId === input.consultorId
        );
    ...
    submissions: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        if (!isAdmin && !isMentor) {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "Acesso restrito a administradores e mentores",
          });
        }
        const filters = { ...input };
        if (isMentor) {
        ...
        const result = await Promise.all(
          sessions.map(async s => {
            const task = s.taskId
              ? await db.getTaskLibraryById(s.taskId)
              : null;
            const aluno = alunoMap.get(s.alunoId);
            const consultor = consultorMap.get(s.consultorId);
            const program = aluno?.programId
    ...
    submissionDetail: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        if (isMentor && session.consultorId !== ctx.user.consultorId) {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "Você só pode ver atividades dos seus alunos",
          });
        }
        const task = session.taskId
          ? await db.getTaskLibraryById(session.taskId)
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (isMentor) {
          ...
          if (!session || session.consultorId !== ctx.user.consultorId) {
            throw new TRPCError({
              code: "FORBIDDEN",
              message: "Você só pode comentar atividades dos seus alunos",
            });
          }
        }
        const authorRole = isAdmin ? "admin" : "mentor";
        const authorName =
          ctx.user.name || (isAdmin ? "Administrador" : "Mentor");
        const id = await db.addActivityComment({
          sessionId: input.sessionId,
          authorId: ctx.user.id,
          authorRole,
  ...
  admin: router({
    ...
    updateAcessoMentor: adminProcedure
      ...
      .mutation(async ({ input }) => {
        return await db.updateConsultorAccess(
          input.consultorId,
          input.loginId,
          input.canLogin,
          "mentor"
    ...
    checkAvailabilityNext10Days: protectedProcedure
      .input(z.object({ consultorId: z.number() }))
      .query(async ({ input }) => {
        const hasAvailability = await db.checkMentorHasAvailabilityNext10Days(
          input.consultorId
        );
      ...
      .mutation(async ({ input }) => {
        ...
        for (const rule of input.rules) {
          if (rule.sessionTo < rule.sessionFrom) {
            throw new Error(
              `Sessão final (${rule.sessionTo}) não pode ser menor que sessão inicial (${rule.sessionFrom})`
            );
          }
        }
        // Validar que não há sobreposição de faixas
        const sorted = [...input.rules].sort(
          (a, b) => a.sessionFrom - b.sessionFrom
        );
        for (let i = 1; i < sorted.length; i++) {
          if (sorted[i].sessionFrom <= sorted[i - 1].sessionTo) {
            throw new Error(
              `Faixas de sessão não podem se sobrepor: sessões ${sorted[i - 1].sessionFrom}-${sorted[i - 1].sessionTo} e ${sorted[i].sessionFrom}-${sorted[i].sessionTo}`
            );
          }
        }
    ...
    createAluno: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        try {
          const { sendEmail, buildOnboardingInviteEmail } = await import(
            "./emailService"
          );
          ...
          console.warn(
            "[Cadastro] Erro ao enviar email de boas-vindas:",
            emailErr
          );
        }
    ...
    updateAluno: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (contratoInicio !== undefined)
          updateData.contratoInicio = contratoInicio
            ? new Date(contratoInicio)
            : null;
        if (contratoFim !== undefined)
          updateData.contratoFim = contratoFim ? new Date(contratoFim) : null;
    ...
    createAccessUser: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (isMentor && userData.role === "manager") {
          ...
          const mentorId =
            "id" in mentorResult ? (mentorResult.id as number) : undefined;
          if (!mentorId) {
    ...
    createAlunoDireto: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        try {
          ...
          console.warn(
            "[Cadastro Direto] Erro ao enviar email de boas-vindas:",
            emailErr
          );
        }
    ...
    deleteAluno: adminProcedure
      .input(
        z.object({
          alunoId: z.number(),
          confirmCascade: z.boolean().default(false),
        })
      )
      .mutation(async ({ input }) => {
    ...
    listMentoringSessions: adminProcedure
      ...
      .query(async ({ input }) => {
        ...
        const {
          mentoringSessions,
          alunos: alunosTable,
          consultors: consultorsTable,
          turmas: turmasTable,
          programs: programsTable,
          trilhas: trilhasTable,
        ...
        if (filters.consultorId)
          conditions.push(
            eq(mentoringSessions.consultorId, filters.consultorId)
          );
        if (filters.turmaId)
        ...
        if (filters.programId && !filters.turmaId) {
          const turmasForProgram = await dbInstance
            .select({ id: turmasTable.id })
            .from(turmasTable)
            .where(eq(turmasTable.programId, filters.programId));
          const turmaIds = turmasForProgram.map(t => t.id);
          if (turmaIds.length > 0) {
            conditions.push(
              sql`${mentoringSessions.turmaId} IN (${sql.raw(turmaIds.join(","))})`
            );
          } else {
        ...
        let query = dbInstance
          ...
          .where(whereClause)
          .orderBy(
            asc(mentoringSessions.sessionNumber),
            desc(mentoringSessions.sessionDate),
            desc(mentoringSessions.id)
          )
          .limit(pageSize)
          .offset(offset);
        ...
        const consultorIds = Array.from(
          new Set(sessions.filter(s => s.consultorId).map(s => s.consultorId!))
        );
        const turmaIds = Array.from(
          new Set(sessions.filter(s => s.turmaId).map(s => s.turmaId!))
        );
        const trilhaIds = Array.from(
          new Set(sessions.filter(s => s.trilhaId).map(s => s.trilhaId!))
        );

        const alunosList =
          alunoIds.length > 0
            ? await dbInstance
                .select({ id: alunosTable.id, name: alunosTable.name })
                .from(alunosTable)
                .where(
                  sql`${alunosTable.id} IN (${sql.raw(alunoIds.join(","))})`
                )
            : [];
        const consultorsList =
          consultorIds.length > 0
            ? await dbInstance
                .select({ id: consultorsTable.id, name: consultorsTable.name })
                .from(consultorsTable)
                .where(
                  sql`${consultorsTable.id} IN (${sql.raw(consultorIds.join(","))})`
                )
            : [];
        const turmasList =
          turmaIds.length > 0
            ? await dbInstance
                .select({ id: turmasTable.id, name: turmasTable.name })
                .from(turmasTable)
                .where(
                  sql`${turmasTable.id} IN (${sql.raw(turmaIds.join(","))})`
                )
            : [];
        const trilhasList =
          trilhaIds.length > 0
            ? await dbInstance
                .select({ id: trilhasTable.id, name: trilhasTable.name })
                .from(trilhasTable)
                .where(
                  sql`${trilhasTable.id} IN (${sql.raw(trilhaIds.join(","))})`
                )
            : [];
        ...
        const enrichedSessions = sessions.map(s => ({
          ...
          consultorNome: s.consultorId
            ? consultorMap.get(s.consultorId) || "Desconhecido"
            : null,
    ...
    updateSessionDate: adminProcedure
      .input(
        z.object({
          ...
          consultorId: z.number().optional(),
          taskStatus: z
            .enum(["entregue", "nao_entregue", "sem_tarefa", "validada"])
            .optional(),
          presence: z.enum(["presente", "ausente"]).optional(),
        })
      )
      .mutation(async ({ input }) => {
        ...
        if (input.sessionNumber !== undefined) {
          const session = await db.getMentoringSessionById(input.sessionId);
          if (session) {
            const existingSessions = await db.getMentoringSessionsByAluno(
              session.alunoId
            );
            const duplicate = existingSessions.find(
              (s: any) =>
                s.sessionNumber === input.sessionNumber &&
        ...
        if (input.sessionDate !== undefined)
          updateData.sessionDate = input.sessionDate;
        if (input.sessionNumber !== undefined)
          updateData.sessionNumber = input.sessionNumber;
        if (input.consultorId !== undefined)
          updateData.consultorId = input.consultorId;
        if (input.taskStatus !== undefined)
          updateData.taskStatus = input.taskStatus;
        if (input.presence !== undefined) updateData.presence = input.presence;

        const success = await db.updateMentoringSession(
          input.sessionId,
          updateData
        );
        return { success };
    ...
    adminCreateSession: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const duplicate = existingSessions.find(
          s => s.sessionNumber === input.sessionNumber
        );
        if (duplicate) {
          throw new TRPCError({
            code: "CONFLICT",
        ...
        try {
          ...
          console.error(
            "[AdminCreateSession] Erro ao enviar notificação por e-mail:",
            emailError
          );
        }
    ...
    updateAllAlunosPlataformaAulas: adminProcedure.mutation(async () => {
  ...
  assessment: router({
      ...
      .mutation(async ({ input }) => {
        ...
        if (alunoCheck.isActive === 0) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message:
              "Não é possível criar PDI para aluno inativo. Ative o aluno primeiro.",
          });
        }

        // ===== FIX: Verificar se já existe uma trilha ativa para este aluno =====
        const existingPdi = await db.getExistingActivePdiByTrilha(
          input.alunoId,
          input.trilhaId
        );
        ...
        if (existingPdi) {
          ...
          console.log(
            `[Assessment] Trilha ${input.trilhaId} já existe para aluno ${input.alunoId} (PDI #${existingPdi.pdiId}). Adicionando ${competencias.length} competência(s) ao existente.`
          );

          const added = await db.addCompetenciasToExistingAssessment(
            existingPdi.pdiId,
            competencias
          );
          ...
          if (added === 0) {
            throw new TRPCError({
              code: "BAD_REQUEST",
              message:
                "Todas as competências selecionadas já existem nesta trilha. Nenhuma nova competência foi adicionada.",
            });
          }
          ...
          try {
            ...
            /* notificação não deve bloquear */
          }

          return {
            success: true,
            pdiId: existingPdi.pdiId,
        ...
        if (pdiData.macroInicio >= pdiData.macroTermino) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message:
              "Data de início do macro ciclo deve ser anterior à data de término",
          });
        }
        ...
        try {
          ...
          /* sync não deve bloquear criação */
        }
        ...
        try {
          ...
          /* notificação não deve bloquear criação */
        }
        ...
        try {
          ...
          if (alunoForEmail) {
            ...
            console.log(
              `[Onboarding Step] Enviando email de avanço (PDI) para admin=${adminEmail}, cc=dina@ckmtalents.net, aluno=${alunoForEmail.name}`
            );
            const stepResult = await sendEmailStep({
            ...
            console.log(
              `[Onboarding Step] Resultado envio (PDI): ${JSON.stringify(stepResult)}`
            );

            // 2) Email para o ALUNO convidando a acessar e assinar
            if (alunoForEmail.email) {
              const mentorInfo = input.consultorId
                ? await db.getConsultorById(input.consultorId)
                : null;
          ...
          console.warn("[Onboarding] Erro ao enviar email de avanço (PDI):", e);
        }
    ...
    congelar: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        await db.congelarAssessmentPdi(
          input.pdiId,
          input.consultorId,
          input.motivo
        );
    ...
    atualizar: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (
          data.macroInicio &&
          data.macroTermino &&
          data.macroInicio >= data.macroTermino
        ) {
          throw new TRPCError({
            code: "BAD_REQUEST",
    ...
    atualizarCompetencias: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        for (const comp of input.updated) {
          ...
          if (comp.nivelAtual != null)
            extraUpdates.nivelAtual = String(comp.nivelAtual);
          if (comp.metaFinal != null)
            extraUpdates.metaFinal = String(comp.metaFinal);
          if (comp.metaCiclo1 != null)
            extraUpdates.metaCiclo1 = String(comp.metaCiclo1);
          if (comp.metaCiclo2 != null)
            extraUpdates.metaCiclo2 = String(comp.metaCiclo2);
          if (comp.justificativa)
            extraUpdates.justificativa = comp.justificativa;
          if (Object.keys(extraUpdates).length > 0) {
            await db.updateAssessmentCompetenciaFields(
              comp.assessmentCompetenciaId,
              extraUpdates
            );
          }
          updatedCount++;
        ...
        for (const comp of input.added) {
          await db.addCompetenciaToAssessment(input.assessmentPdiId, {
            ...
            microTermino: comp.microTermino,
            nivelAtual:
              comp.nivelAtual != null ? String(comp.nivelAtual) : null,
            metaFinal: comp.metaFinal != null ? String(comp.metaFinal) : null,
            metaCiclo1:
              comp.metaCiclo1 != null ? String(comp.metaCiclo1) : null,
            metaCiclo2:
              comp.metaCiclo2 != null ? String(comp.metaCiclo2) : null,
    ...
    create: adminProcedure
      .input(
        z.object({
          ...
          targetAudience: z
            .enum(["all", "sebrae_to", "sebrae_acre", "embrapii", "banrisul"])
            .optional(),
          status: z
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const endDate = input.endDate
          ...
          : new Date(eventDate.getTime() + (input.duration || 60) * 60000);
    ...
    sendReminder: adminProcedure
      .input(
        z.object({
          webinarId: z.number(),
          recipients: z
            .array(z.enum(["alunos", "gerentes", "mentores"]))
            .min(1, "Selecione pelo menos um grupo de destinatários"),
        })
      )
      .mutation(async ({ input }) => {
        ...
        if (uniqueRecipients.length === 0) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message:
              "Nenhum destinatário com email cadastrado encontrado nos grupos selecionados",
          });
        }

        // Format event date and time in Brazil timezone
        const eventDate = webinar.eventDate
        ...
        const studentRecipients = uniqueRecipients.filter(
          r => r.group === "aluno"
        );
        ...
        try {
          ...
          console.log(
            `[Reminder] ${inAppNotifications.length} notificações in-app criadas para webinar ${webinar.title}`
          );
        } catch (err: any) {
          console.error(
            `[Reminder] Erro ao criar notificações in-app:`,
            err.message
          );
        }

        // === 2. SEND EMAILS VIA SMTP TO ALL STUDENTS ===
        const { sendEmail, buildWebinarReminderEmail } = await import(
          "./emailService"
        );
        ...
        console.log(
          `[Reminder] Emails enviados: ${emailsSent}/${uniqueRecipients.length}, falhas: ${emailsFailed}`
        );
        if (errors.length > 0) {
          console.error(
            `[Reminder] Erros de email:`,
...
Destinatários: ${Object.entries(groupCounts)
              .map(([k, v]) => `${v} ${k}`)
              .join(", ")}
...
Erros: ${errors.slice(0, 3).join("; ")}`
    ...
    update: adminProcedure
      .input(
        z.object({
          ...
          content: z.string().optional(),
          type: z
            .enum(["webinar", "course", "activity", "notice", "news"])
            .optional(),
          imageUrl: z.string().optional(),
          actionUrl: z.string().optional(),
          actionLabel: z.string().optional(),
  ...
  attendance: router({
    ...
    markPresence: protectedProcedure
      .input(
        z.object({
          ...
          reflexao: z
            .string()
            .min(20, "A reflexão deve ter pelo menos 20 caracteres"),
        })
      )
      .mutation(async ({ input, ctx }) => {
        ...
        const matchingWebinar = allWebinars.find(
          (w: any) =>
            w.title?.toLowerCase().trim() ===
            eventRecord.title?.toLowerCase().trim()
        );
        if (matchingWebinar) {
          ...
          if (matchingWebinar.status !== "completed") {
            ...
            const startDate =
              matchingWebinar.startDate || matchingWebinar.eventDate;
            if (startDate && new Date(startDate) > new Date()) {
              throw new TRPCError({
                code: "BAD_REQUEST",
                message:
                  "A marcação de presença só é liberada após o início do evento.",
              });
            }
          }
        }
        ...
        const result = await db.markWebinarAttendance(
          aluno.id,
          realEventId,
          input.reflexao
        );
    ...
    myAttendance: protectedProcedure.query(async ({ ctx }) => {
      ...
      const eventsArr = await Promise.all(
        eventIds.map(id => db.getEventById(id))
      );
      const allScheduled = await db.listWebinars();
      const eventMap = new Map<
        number,
        NonNullable<Awaited<ReturnType<typeof db.getEventById>>>
      >();
      for (const e of eventsArr) {
        if (e) eventMap.set(e.id, e);
      }
      const scheduledByTitle = new Map(
        allScheduled.map(sw => [sw.title?.toLowerCase().trim(), sw])
      );
      ...
      return participations
        ...
        .map(p => {
          ...
          const matchedWebinar = evt
            ? scheduledByTitle.get(evt.title?.toLowerCase().trim() || "")
            : null;
          return {
            eventId: p.eventId,
    ...
    myTasks: protectedProcedure.query(async ({ ctx }) => {
      ...
      const sessionsWithTask = sessions.filter(s => {
        ...
        if (s.taskMode === "personalizada" || s.taskMode === "livre")
          return true;
      ...
      const tasks = await Promise.all(
        sessionsWithTask.map(async s => {
          ...
          const taskOQueFazer =
            task?.oQueFazer || s.customTaskDescription || "";
    ...
    submitEvidence: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!session || session.alunoId !== aluno.id) {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "Sessão não pertence a este aluno",
          });
        }
        ...
        if (session.taskStatus === "validada") {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Atividade já foi validada, não pode ser alterada",
          });
        }
        ...
        if (!input.evidenceLink && !input.evidenceImageBase64) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "Envie pelo menos um link ou uma imagem como evidência",
          });
        }
        ...
        if (input.evidenceImageBase64) {
          ...
          const ext =
            input.evidenceImageName?.split(".").pop()?.toLowerCase() || "jpg";
    ...
    submitAplicabilidade: protectedProcedure
      .input(
        z.object({
          sessionId: z.number(),
          textoAplicabilidade: z
            .string()
            .min(1, "Descreva como aplicou na prática"),
    ...
    update: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (data.periodoInicio)
          updateData.periodoInicio = new Date(data.periodoInicio);
        if (data.periodoTermino)
          updateData.periodoTermino = new Date(data.periodoTermino);
    ...
    updateNivel: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (input.nivelAtual !== undefined)
          updates.nivelAtual = String(input.nivelAtual);
        if (input.metaCiclo1 !== undefined)
          updates.metaCiclo1 = String(input.metaCiclo1);
        if (input.metaCiclo2 !== undefined)
          updates.metaCiclo2 = String(input.metaCiclo2);
        if (input.metaFinal !== undefined)
          updates.metaFinal = String(input.metaFinal);
        if (input.justificativa !== undefined)
          updates.justificativa = input.justificativa;
    ...
    enviar: protectedProcedure
      .input(
        z.object({
          ...
          oQueAprendi: z
            .string()
            .min(1, 'Campo "O que aprendi" \u00e9 obrigat\u00f3rio'),
          oQueMudei: z
            .string()
            .min(1, 'Campo "O que mudei" \u00e9 obrigat\u00f3rio'),
          resultadoMensuravel: z
            .string()
            .min(
              1,
              'Campo "Resultado mensur\u00e1vel" \u00e9 obrigat\u00f3rio'
            ),
          antesVsDepois: z
            .string()
            .min(1, 'Campo "Antes vs. Depois" \u00e9 obrigat\u00f3rio'),
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (
          input.evidenciaBase64 &&
          input.evidenciaFileName &&
          input.evidenciaMimeType
        ) {
        ...
        const caseExistente = casesExistentes.find(
          c => c.trilhaId === input.trilhaId
        );
        ...
        try {
          ...
          const mentorDoAluno = aluno.consultorId
            ? allConsultors.find(c => c.id === aluno.consultorId)
            : null;
          const gestoresDoPrograma = allConsultors.filter(
            c => c.role === "gerente" && c.managedProgramId === aluno.programId
          );
          ...
          if (mentorDoAluno?.email) {
            const mentorUser = allUsers.find(
              u => u.email?.toLowerCase() === mentorDoAluno.email?.toLowerCase()
            );
          ...
          for (const gestor of gestoresDoPrograma) {
            if (gestor.email) {
              const gestorUser = allUsers.find(
                u => u.email?.toLowerCase() === gestor.email?.toLowerCase()
              );
              if (gestorUser && !usersToNotify.includes(gestorUser.id))
                usersToNotify.push(gestorUser.id);
            }
          }
          ...
          console.error(
            "Erro ao enviar notifica\u00e7\u00f5es do Relat\u00f3rio de Impacto:",
            notifError
          );
  ...
  metas: router({
    ...
    porCompetencia: protectedProcedure
      ...
      .query(async ({ input }) => {
        return await db.getMetasByCompetencia(
          input.alunoId,
          input.assessmentCompetenciaId
        );
      }),
    ...
    resumoTodos: protectedProcedure.query(async () => {
    ...
    minhas: protectedProcedure.query(async ({ ctx }) => {
      ...
      if (!alunoId)
        return {
          alunoId: null,
          metas: [],
          resumo: { total: 0, cumpridas: 0, percentual: 0, porCompetencia: [] },
        };
      ...
      .query(async ({ input }) => {
        ...
        if (input?.competencia) {
          return all.filter(t =>
            t.competencia
              .toLowerCase()
              .includes(input.competencia!.toLowerCase())
          );
        }
        return all;
      }),
...
{"titulo": "Título curto do desafio (máx 80 caracteres)", "descricao": "Descrição detalhada do desafio, explicando o que fazer, como fazer e o que se espera como resultado (2-3 frases)"}`,
            ...
            json_schema: {
              ...
              schema: {
                ...
                properties: {
                  titulo: {
                    type: "string",
                    description: "Título curto do desafio",
                  },
                  descricao: {
                    type: "string",
        ...
        if (!content)
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message: "Falha ao gerar sugestão com IA",
          });
        const contentStr =
  ...
  disc: router({
    ...
    salvarRespostas: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        try {
          const aluno = await db.getAlunoById(input.alunoId);
          if (aluno) {
            const { sendEmail, buildOnboardingStepEmail } = await import(
              "./emailService"
            );
            const adminEmail = process.env.SMTP_USER || "";
            ...
            console.log(
              `[Onboarding Step] Enviando email de avanço (DISC) para admin=${adminEmail}, cc=dina@ckmtalents.net, aluno=${aluno.name}`
            );
            const result = await sendEmail({
              to: adminEmail || "dina@ckmtalents.net",
              cc: adminEmail ? "dina@ckmtalents.net" : undefined,
            ...
            console.log(
              `[Onboarding Step] Resultado envio (DISC): ${JSON.stringify(result)}`
            );
          }
        } catch (e) {
          console.warn(
            "[Onboarding] Erro ao enviar email de avanço (DISC):",
            e
          );
        }
      ...
      .query(async ({ input }) => {
        ...
        return {
          cicloInicial: {
            ...
            scores: {
              D: Number(primeiro.scoreD),
              I: Number(primeiro.scoreI),
              S: Number(primeiro.scoreS),
              C: Number(primeiro.scoreC),
            },
          ...
          cicloAtual: {
            ...
            scores: {
              D: Number(ultimo.scoreD),
              I: Number(ultimo.scoreI),
              S: Number(ultimo.scoreS),
              C: Number(ultimo.scoreC),
            },
    ...
    verificarReassessment: protectedProcedure
      ...
      .query(async ({ input }) => {
        ...
        const contratoAtivo =
          contratos.find((c: any) => c.status === "ativo") ||
          contratos[contratos.length - 1];
        const termino = contratoAtivo.periodoTermino
          ? new Date(contratoAtivo.periodoTermino)
          : null;
        ...
        if (!termino) {
          return {
            elegivel: false,
            motivo: "Contrato sem data de término definida",
          };
        }

        // Elegível se o contrato já venceu ou está a menos de 30 dias do vencimento
        const diasParaVencimento = Math.ceil(
          (termino.getTime() - agora.getTime()) / (1000 * 60 * 60 * 24)
        );
        ...
        return {
          ...
          motivo: elegivel
            ? diasParaVencimento <= 0
              ? "Contrato finalizado"
              : `Faltam ${diasParaVencimento} dias para o término`
  ...
  autopercepção: router({
    ...
    salvar: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        await db.saveAutopercepcoes(
          input.alunoId,
          input.avaliacoes.map(a => ({
            alunoId: input.alunoId,
  ...
  onboarding: router({
    ...
    salvarCadastro: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (onbStatus.hasPdi && !onbStatus.needsOnboarding) {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "Onboarding em modo somente leitura. Aluno já possui PDI.",
          });
        }
        ...
        try {
          ...
          if (aluno) {
            ...
            console.log(
              `[Onboarding Step] Enviando email de avanço (Cadastro) para admin=${adminEmail}, cc=dina@ckmtalents.net, aluno=${aluno.name}`
            );
            const result = await sendEmail({
              to: adminEmail || "dina@ckmtalents.net",
              cc: adminEmail ? "dina@ckmtalents.net" : undefined,
            ...
            console.log(
              `[Onboarding Step] Resultado envio (Cadastro): ${JSON.stringify(result)}`
            );
          }
        } catch (e) {
          console.warn(
            "[Onboarding] Erro ao enviar email de avanço (cadastro):",
            e
          );
        }
    ...
    escolherMentora: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        try {
          ...
          console.warn(
            "[Onboarding] Erro ao enviar notificação de novo aluno para mentora:",
            emailErr
          );
        }
    ...
    criarAgendamento: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (onbStatus.hasPdi && !onbStatus.needsOnboarding) {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "Onboarding em modo somente leitura.",
          });
        }
        ...
        const result = await db.createGroupAppointment({
          ...
          description:
            notes || "Primeiro encontro de mentoria agendado pelo onboarding",
          scheduledDate,
          startTime,
          endTime,
        ...
        try {
          ...
          console.warn(
            "[Onboarding] Erro ao enviar notificação de agendamento para mentora:",
            emailErr
          );
        }
        ...
        try {
          ...
          console.warn(
            "[Onboarding] Erro ao enviar confirmação de agendamento para aluno:",
            emailErr
          );
        }
        ...
        try {
          if (aluno) {
            const { sendEmail: sendEmailStep, buildOnboardingStepEmail } =
              await import("./emailService");
            ...
            console.log(
              `[Onboarding Step] Enviando email de avanço (Agendamento) para admin=${adminEmailStep}, cc=dina@ckmtalents.net, aluno=${aluno.name}`
            );
            const result = await sendEmailStep({
              to: adminEmailStep || "dina@ckmtalents.net",
              cc: adminEmailStep ? "dina@ckmtalents.net" : undefined,
              subject: emailData.subject,
              html: emailData.html,
              text: emailData.text,
            });
            console.log(
              `[Onboarding Step] Resultado envio (Agendamento): ${JSON.stringify(result)}`
            );
          }
        } catch (e) {
          console.warn(
            "[Onboarding] Erro ao enviar email de avanço (agendamento):",
            e
          );
        }
    ...
    progresso: protectedProcedure
      ...
      .query(async ({ input }) => {
        ...
        const relatorioFeito = sessoes.some(
          s => s.presence === "presente" && (s.feedback || s.notaEvolucao)
        );
        ...
        const encontroData = sessaoRealizada?.sessionDate
          ? String(sessaoRealizada.sessionDate)
          : null;
        ...
        const todosVideosAssistidos = !!(
          jornada?.videoBoasVindas &&
          jornada?.videoCompetencias &&
          jornada?.videoWebinars &&
          jornada?.videoTarefas &&
          jornada?.videoMetas
        );
        ...
        if (contratoAtivo) {
          contratoTermino = contratoAtivo.periodoTermino
            ? String(contratoAtivo.periodoTermino)
            : null;
        ...
        if (
          ...
          mentoraEscolhida &&
          agendamentoFeito &&
          (encontroRealizado || todosVideosAssistidos)
        )
          step = 6;
        ...
        return {
          ...
          jornada: jornada
            ? {
                ...
                aceiteRealizadoEm: jornada.aceiteRealizadoEm
                  ? jornada.aceiteRealizadoEm.getTime()
                  : null,
              }
            : null,
        };
    ...
    marcarVideoAssistido: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const todosAssistidos = Object.values(videoStates).every(
          (v: any) => v === 1
        );
        if (todosAssistidos) {
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        try {
          ...
          if (aluno) {
            const {
              sendEmail,
              buildAceiteParabensEmail,
              buildAceiteNotificacaoEmail,
            } = await import("./emailService");
            const adminEmail = process.env.SMTP_USER || "";
            const loginUrl =
              process.env.VITE_OAUTH_PORTAL_URL ||
              "https://gestaodash-5n7arrgn.manus.space";
            ...
            if (aluno.email) {
              ...
              console.log(
                `[Onboarding Aceite] Enviando email de parabéns para aluno=${aluno.email}`
              );
              await sendEmail({
                to: aluno.email,
                subject: parabensData.subject,
                html: parabensData.html,
                text: parabensData.text,
              });
            }
            ...
            const ccList = [mentorEmail, "dina@ckmtalents.net"]
              .filter(Boolean)
              .join(", ");
            console.log(
              `[Onboarding Aceite] Enviando notificação para admin=${adminEmail}, cc=${ccList}`
            );
            await sendEmail({
              to: adminEmail || "dina@ckmtalents.net",
              cc: ccList || undefined,
          ...
          console.warn("[Onboarding] Erro ao enviar emails de aceite:", e);
        }
    ...
    solicitarRevisaoAceite: protectedProcedure
      .input(
        z.object({
          ...
          justificativa: z
            .string()
            .min(5, "Por favor, explique o que gostaria de rever."),
        })
      )
      .mutation(async ({ input, ctx }) => {
        const onbStatus = await db.getAlunoOnboardingStatus(ctx.user);
        ...
        const totalRevisoes = await db.onboardingRevisoesDb.countByAluno(
          input.alunoId
        );
        if (totalRevisoes >= 5) {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "Você atingiu o limite de 5 solicitações de revisão.",
          });
        }
        ...
        try {
          const aluno = await db.getAlunoById(input.alunoId);
          if (aluno) {
            const { sendEmail, buildRevisaoAceiteEmail } = await import(
              "./emailService"
            );
            const adminEmail = process.env.SMTP_USER || "";
            const loginUrl =
            ...
            const ccList = [mentorEmail, "dina@ckmtalents.net"]
              ...
              .join(", ");
            console.log(
              `[Onboarding Revisão] Enviando email de solicitação de revisão para admin=${adminEmail}, cc=${ccList}, aluno=${aluno.name}`
            );
            await sendEmail({
              to: adminEmail || "dina@ckmtalents.net",
              cc: ccList || undefined,
              subject: revisaoData.subject,
              html: revisaoData.html,
              text: revisaoData.text,
            });
            emailEnviado = true;
          }
        } catch (e) {
          console.warn("[Onboarding] Erro ao enviar email de revisão:", e);
        }
        ...
        console.log(
          `[Onboarding Revisão] Solicitação registrada no banco: id=${revisao.id}, alunoId=${input.alunoId}`
        );
        ...
        try {
          ...
          if (aluno?.consultorId) {
            const allUsers = await db.getAllUsers();
            const mentorUser = allUsers.find(
              (u: any) => u.consultorId === aluno.consultorId
            );
            if (mentorUser) {
              notificacoes.push({
          ...
          if (notificacoes.length > 0) {
            ...
            console.log(
              `[Onboarding Revisão] ${notificacoes.length} notificações in-app criadas`
            );
          }
        } catch (notifErr) {
          console.warn(
            "[Onboarding Revisão] Erro ao criar notificações in-app:",
            notifErr
          );
        }
        ...
        return {
          success: true,
          message:
            "Sua solicitação de revisão foi enviada para a mentora e administração.",
    ...
    listarRevisoes: managerProcedure
      .input(
        z
          .object({
            status: z
              .enum(["pendente", "em_analise", "resolvida", "cancelada"])
              .optional(),
          })
          .optional()
      )
      .query(async ({ ctx, input }) => {
        ...
        const consultorId =
          ctx.user.role === "admin" ? undefined : (ctx.user as any).consultorId;
        return await db.onboardingRevisoesDb.getEnriquecidas(
          input?.status,
          consultorId
        );
      }),
...
Responda APENAS em JSON com o formato especificado.`,
            ...
            json_schema: {
              ...
              schema: {
                ...
                properties: {
                  nome: {
                    type: "string",
                    description: "Nome curto e descritivo da tarefa",
                  },
                  resumo: {
                    type: "string",
                    description: "Resumo breve do objetivo da tarefa",
                  },
                  oQueFazer: {
                    type: "string",
                    description:
                      "Descrição detalhada passo a passo do que fazer",
                  },
                  oQueGanha: {
                    type: "string",
                    description:
                      "Benefícios e aprendizados ao realizar a tarefa",
                  },
                },
        ...
        if (!content)
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message: "Falha ao gerar tarefa com IA",
          });
        const contentStr =
          typeof content === "string" ? content : JSON.stringify(content);
        try {
    ...
    create: adminProcedure
      .input(
        z.object({
          ...
          tipo: z
            .enum(["gratuito", "online_pago", "presencial"])
            .default("gratuito"),
  ...
  activities: router({
    ...
    update: adminProcedure
      .input(
        z.object({
          ...
          descricao: z.string().optional(),
          tipo: z
            .enum(["workshop", "treinamento", "palestra", "evento", "outro"])
            .optional(),
    ...
    myRegistration: protectedProcedure
      ...
      .query(async ({ input, ctx }) => {
        return db.getRegistrationByUserAndActivity(
          ctx.user.id,
          input.activityId
        );
      }),
    ...
    register: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const existing = await db.getRegistrationByUserAndActivity(
          ctx.user.id,
          input.activityId
        );
        if (existing)
  ...
  alertasMentoria: router({
    ...
    enviarAlertas: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const { sendEmail, buildMentoringAlertEmail } = await import(
          "./emailService"
        );
        const { ENV } = await import("./_core/env");
        ...
        const allAlunos = allAlunosRaw.filter(
          a => !a.programId || activeProgramIds.has(a.programId)
        );
        ...
        const { mentoringSessions: msTable } = await import(
          "../drizzle/schema"
        );
        const allSessions = await dbInstance.select().from(msTable);

        // Calculate last session per aluno (with any mentor)
        const lastSessionByAluno = new Map<
          number,
          { date: Date; consultorId: number }
        >();
        for (const session of allSessions) {
          ...
          if (!current || sessionDate > current.date) {
            lastSessionByAluno.set(session.alunoId, {
              date: sessionDate,
              consultorId: session.consultorId,
            });
          }
        }
        ...
        for (const aluno of allAlunos) {
          ...
          // Get current mentor
          const mentor = aluno.consultorId
            ? consultorMap.get(aluno.consultorId)
            : null;
          ...
          if (lastSession) {
            diasSemSessao = Math.floor(
              (now - lastSession.date.getTime()) / (1000 * 60 * 60 * 24)
            );
          ...
          if (diasSemSessao >= diasMinimo) {
            ...
            if (!dryRun && aluno.email) {
              try {
                ...
                const ccList = [mentor.email, ENV.smtpUser]
                  .filter(Boolean)
                  .join(", ");
  ...
  vencimentoCiclo: router({
    ...
    enviarAlertas: adminProcedure
      ...
      .mutation(async ({ input }) => {
        const { verificarEEnviarAlertasVencimentoCiclo } = await import(
          "./cronVencimentoCiclo"
        );
  ...
  onboardingTracking: router({
    ...
    resendInvite: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const allPrograms = await db.getPrograms();
        const program = aluno.programId
          ? allPrograms.find(p => p.id === aluno.programId)
          : null;
  ...
  course: router({
    ...
    getCatalog: protectedProcedure
      ...
      .query(async ({ input }) => {
        try {
          const catalog = await db.getCourseCatalog(
            input.alunoId,
            input.microcicloId
          );
          return catalog;
    ...
    startModule: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        try {
          ...
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message: "Erro ao iniciar módulo",
          });
        }
      }),
    ...
    submitReflection: protectedProcedure
      .input(
        z.object({
          ...
          textoRelato: z
            .string()
            .min(100, "Reflexão deve ter no mínimo 100 caracteres"),
        })
      )
      .mutation(async ({ input, ctx }) => {
        try {
          const alunoId = ctx.user.alunoId || 0;
          await db.submitReflection(
            alunoId,
            input.moduloId,
            input.progressoId,
            input.textoRelato
          ...
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message: "Erro ao enviar reflexão",
          });
        }
      }),
    ...
    requestExtension: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        try {
          ...
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message: "Erro ao solicitar prorrogação",
          });
        }
      }),
      ...
      .mutation(async ({ input, ctx }) => {
        try {
          ...
          if (ctx.user.role !== "manager" && ctx.user.role !== "admin") {
            throw new TRPCError({
              code: "FORBIDDEN",
              message: "Apenas mentores podem aprovar prorrogações",
            });
          }
          ...
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message: "Erro ao processar prorrogação",
          });
        }
      }),
     ...
     */
    getMentorPanel: protectedProcedure.query(async ({ ctx }) => {
      try {
        // Verificar se é mentor
        if (ctx.user.role !== "manager" && ctx.user.role !== "admin") {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "Apenas mentores podem acessar este painel",
          });
        }
  ...
  courseAdmin: router({
     ...
     */
    listCursos: adminProcedure.query(async () => {
      try {
        const database = await getDb();
        if (!database) throw new Error("Database not available");
        const cursos = await database.select().from(competencias);
        return cursos;
      } catch (error) {
    ...
    admin: router({
      ...
      listarCursos: protectedProcedure
        ...
        .query(async ({ input }) => {
          ...
          return await database
            ...
            .orderBy(
              asc(cursosCompetencias.ordem),
              asc(cursosCompetencias.titulo)
            );
        }),

      listarTodosCursos: protectedProcedure.query(async () => {
        const database = await db.getDb();
        if (!database) return [];
        ...
        return await database
          ...
          .from(cursosCompetencias)
          .orderBy(
            asc(cursosCompetencias.competenciaId),
            asc(cursosCompetencias.ordem),
            asc(cursosCompetencias.titulo)
          );
      }),
      ...
      criarAtividade: adminProcedure
        ...
        .mutation(async ({ input }) => {
          try {
            ...
            const urlFinal =
              input.urlGenially?.trim() || input.urlMidia?.trim() || null;
            const descricao = input.descricao?.trim() || null;
            const ordem = Number(input.ordem ?? 0);

            const tempoMinimo =
              input.tempoMinimoObrigatorioSegundos != null &&
              input.tempoMinimoObrigatorioSegundos > 0
      ...
      atualizarAtividade: adminProcedure
        ...
        .mutation(async ({ input }) => {
          ...
          if (input.urlGenially !== undefined)
            updates.urlGenially = input.urlGenially ?? null;
          ...
          if (input.imagemUrl !== undefined)
            updates.imagemUrl = input.imagemUrl || null;
      ...
      obterAtividadeDetalhes: protectedProcedure
        ...
        .query(async ({ input }) => {
          ...
          const result = await database
            ...
            .leftJoin(
              avaliacoesAtividade,
              eq(atividadesCurso.id, avaliacoesAtividade.atividadeId)
            )
            .where(eq(atividadesCurso.id, input.atividadeId))
            .limit(1);
      ...
      listarAvaliacoesCurso: protectedProcedure
        ...
        .query(async ({ input }) => {
          ...
          return await database
            ...
            .innerJoin(
              atividadesCurso,
              eq(avaliacoesAtividade.atividadeId, atividadesCurso.id)
            )
            .where(
              and(
                eq(atividadesCurso.cursoId, input.cursoId),
    ...
    mentor: router({
      ...
      editarAtribuicao: protectedProcedure
        .input(
          z.object({
            ...
            dataPrazo: z.string().optional(),
            status: z
              .enum(["nao_iniciado", "em_progresso", "concluido", "prorrogado"])
              .optional(),
          })
        )
        .mutation(async ({ ctx, input }) => {
      ...
      liberarTentativas: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
          ...
          const progressos = await database
            ...
            .where(
              and(
                ...
                eq(
                  alunoAtividadeProgresso.cursoAtribuidoId,
                  input.cursoAtribuidoId
                )
              )
            );
          ...
          return {
            success: true,
            atividadesResetadas: progressos.filter(
              p => p.status === "bloqueada" || p.status === "reprovada"
            ).length,
          };
        }),
      ...
      acompanharProgresso: protectedProcedure
        ...
        .query(async ({ input }) => {
          ...
          return await database
            ...
            .leftJoin(
              competenciasModulos,
              eq(alunoModuloProgresso.moduloId, competenciasModulos.id)
            )
      ...
      listarCursosAtribuidosAoAluno: protectedProcedure
        ...
        .query(async ({ input }) => {
          ...
          const cursos = await database
            ...
            .leftJoin(
              cursosCompetencias,
              eq(alunoCursoAtribuido.cursoId, cursosCompetencias.id)
            )
            .leftJoin(
              competencias,
              eq(alunoCursoAtribuido.competenciaId, competencias.id)
            )
    ...
    aluno: router({
      ...
      getCursosAtribuidos: protectedProcedure.query(async ({ ctx }) => {
        ...
        const resultados = await database
          ...
          .leftJoin(
            competencias,
            eq(cursosCompetencias.competenciaId, competencias.id)
          )
          .where(eq(alunoCursoAtribuido.alunoId, aluno.id))
          .orderBy(desc(alunoCursoAtribuido.dataAtribuicao));
      ...
      obterAtividadesCurso: protectedProcedure
        ...
        .query(async ({ ctx, input }) => {
          ...
          return atividades.map((atividade, index) => {
            ...
            const avaliacaoAnterior = atividadeAnterior
              ? avaliacaoMap.get(atividadeAnterior.id)
              : null;
            const anteriorSemAvaliacao =
              atividadeAnterior && !avaliacaoAnterior;
            const anteriorAprovada =
              !atividadeAnterior ||
              progressoAnterior?.status === "aprovada" ||
              Number(progressoAnterior?.notaFinal ?? 0) >= 8 ||
              (anteriorSemAvaliacao &&
                progressoAnterior?.status === "concluida");
      ...
      iniciarAtividade: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
          ...
          const resumoTempoAtual = montarResumoTempo(
            atividade,
            existente ?? {
          ...
          if (!existente) {
            await database.insert(alunoAtividadeProgresso).values({
              ...
              tempoCumpridoEm: resumoTempoAtual.tempoCumprido
                ? new Date()
                : null,
              liberadoParaAvaliacaoEm: resumoTempoAtual.liberadoParaAvaliacao
                ? new Date()
                : null,
            ...
            const novoStatus =
              existente.status === "aprovada" ? "aprovada" : "em_andamento";
            ...
            await database
              ...
              .set({
                ...
                tempoMinimoExigidoSegundos,
                avaliacaoLiberada: resumoTempoAtual.liberadoParaAvaliacao
                  ? 1
                  : 0,
                bloqueioPorTempo: resumoTempoAtual.bloqueioPorTempo,
                tempoCumpridoEm:
                ...
                liberadoParaAvaliacaoEm:
                  resumoTempoAtual.liberadoParaAvaliacao &&
                  !existente.liberadoParaAvaliacaoEm
                    ? new Date()
          ...
          const [sessaoAtiva] = await database
            ...
            .where(
              and(
                ...
                eq(
                  sessoesEstudoAtividade.cursoAtribuidoId,
                  input.cursoAtribuidoId
                ),
          ...
          if (!avaliacao) {
            return {
              ...
              tempo: {
                tempoAtivoAcumuladoSegundos:
                  resumoTempoAtual.tempoAtivoAcumuladoSegundos,
                tempoMinimoExigidoSegundos:
                  resumoTempoAtual.tempoMinimoExigidoSegundos,
                tempoRestanteSegundos: resumoTempoAtual.tempoRestanteSegundos,
                percentualTempoCumprido:
                  resumoTempoAtual.percentualTempoCumprido,
                bloqueioPorTempo: resumoTempoAtual.bloqueioPorTempo,
          ...
          const questoesEmbaralhadas = [...todasQuestoes].sort(
            () => 0.5 - Math.random()
          );
      ...
      registrarHeartbeatAtividade: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
          ...
          const segundosParaSomar = normalizarSegundosHeartbeat(
            input.segundosAtivos
          );

          // Anti-duplicidade básica: se o último heartbeat foi há menos de 10 segundos, ignorar soma de tempo
          const agora = new Date();
          const ultimaBatida = progresso.ultimoHeartbeatEm
            ? new Date(progresso.ultimoHeartbeatEm)
            : null;
          const diffSegundos = ultimaBatida
            ? (agora.getTime() - ultimaBatida.getTime()) / 1000
            : 999;
          ...
          await database
            ...
            .set({
              ...
              avaliacaoLiberada: resumo.liberadoParaAvaliacao
                ? 1
                : (progresso.avaliacaoLiberada ?? 0),
              tempoCumpridoEm:
                resumo.tempoCumprido && !progresso.tempoCumpridoEm
                  ? agora
                  : progresso.tempoCumpridoEm,
              liberadoParaAvaliacaoEm:
                resumo.liberadoParaAvaliacao &&
                !progresso.liberadoParaAvaliacaoEm
                  ? agora
                  : progresso.liberadoParaAvaliacaoEm,
              updatedAt: agora,
            })
          ...
          if (sessaoAtiva && deveSomarTempo) {
            await database
              ...
              .set({
                tempoAtivoSegundos:
                  Number(sessaoAtiva.tempoAtivoSegundos ?? 0) +
                  segundosParaSomar,
      ...
      concluirAtividade: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
          ...
          if (!progresso) {
            throw new TRPCError({
              code: "NOT_FOUND",
              message: "Progresso da atividade não encontrado",
            });
          }
          ...
          if (!temAvaliacao) {
            ...
            const atividadeIndex = atividades.findIndex(
              a => a.id === input.atividadeId
            );
            const proximaAtividade =
              atividadeIndex >= 0 && atividadeIndex < atividades.length - 1
            ...
            const todasAprovadas = todasAsAtividades.every(
              a => a.status === "aprovada"
            );

            if (
              todasAprovadas &&
              todasAsAtividades.length === atividades.length
            ) {
              await database
                .update(alunoCursoAtribuido)
                .set({
      ...
      obterAvaliacaoDaAtividade: protectedProcedure
        .input(
          z.object({
            ...
            cursoAtribuidoId: z
              .number()
              .int()
              .min(1, "cursoAtribuidoId inválido"),
        ...
        .query(async ({ ctx, input }) => {
          ...
          if (!atribuicao) {
            throw new TRPCError({
              code: "FORBIDDEN",
              message: "Curso não atribuído a este aluno",
            });
          }
          ...
          if (!atividade) {
            throw new TRPCError({
              code: "NOT_FOUND",
              message: "Atividade não encontrada neste curso",
            });
          }
      ...
      submeterAvaliacao: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
          ...
          if (!progresso.avaliacaoLiberada && !resumo.liberadoParaAvaliacao) {
            throw new TRPCError({
              code: "FORBIDDEN",
              message: "Avaliação não foi liberada para esta atividade",
            });
          }
          ...
          if (bloqueado) {
            try {
              ...
              // Buscar dados do mentor
              const mentorData = cursoAtrib?.mentorId
                ? await db.getConsultorById(cursoAtrib.mentorId)
                : null;
              ...
              const [cursoData] = cursoAtrib?.cursoId
                ? await database
                    .select()
                    .from(cursosCompetencias)
              ...
              if (mentorEmail) {
                await sendEmail({
                  ...
                  console.error("[Email bloqueio] Erro:", err.message)
                );
                console.log(
                  `[Email bloqueio] Enviado para mentor=${mentorEmail}, cc=${adminEmail}, aluno=${alunoNome}`
                );
              } else {
                ...
                console.log(
                  `[Email bloqueio] Enviado para admin=${adminEmail}, aluno=${alunoNome} (sem mentor)`
                );
              }
            } catch (emailError: any) {
              console.error(
                "[Email bloqueio] Falha ao enviar notificação:",
                emailError.message
              );
        ...
        .mutation(async ({ ctx, input }) => {
          ...
          if (tentativaJoin) {
            ...
            const respostasAtuais =
              ...
              typeof tentativa.respostasAluno === "object"
                ? tentativa.respostasAluno
                : {};
      ...
      concluirCurso: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
          ...
          const notaFinal =
            ultimaTentativaJoin?.tentativas_avaliacao?.nota ?? null;
    ...
    obterPorChave: publicProcedure
      ...
      .query(async ({ input }) => {
        ...
        const video = await database
          ...
          .where(
            and(
              eq(onboardingVideos.chave, input.chave),
              eq(onboardingVideos.isActive, 1)
            )
          )
          .limit(1);
        return video[0] || null;
